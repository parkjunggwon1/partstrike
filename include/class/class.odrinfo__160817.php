		...
		while($row = mysql_fetch_array($result)){
			...
			<tr id="tr_<?=$odr_det_idx?>"><!------------ 내용 첫 번째 tr -------------->
						...
						<td colspan="13">
							...
							<table class="detail-table">
								<tbody>
									...
									<?if(strlen($memo) > 0){?>
									<tr class="noinput">
										<td colspan="4" lang="ko"><strong class="c-black" lang="en">Memo : </strong><?=$memo?> </td>
									</tr>
									<?}?>
									<tr>
				...
				<?}elseif ($loadPage== "31_05"){ //------------------------------------------------------------------------------------------------------?>
					...
					<td>
						<input type="text" class="i-txt2 c-blue onlynum numfmt t-rt" maxlength="10" name="odr_quantity[]" odr_det_idx="<?=$odr_det_idx?>" supply_quantity="<?=$supply_quantity;?>" quantity="<?=$quantity + $odr_quantity;?>" amd_yn="Y" value="<?=$odr_quantity==0?"":number_format($odr_quantity)?>" style="width:56px;">
					</td>
				...
				<? //-------------- 송장(30_08) : 판매자 페이지에서 보여지는 송장 목록 -------------------------------
					...
					<td><?=$dc?></td>
					<td><?=$rhtype?></td>
					<td class="t-rt"><?=$origin_qty==0?"":number_format($origin_qty)?></td>
					<?}?>
					<td class="t-rt">$<?=number_format($price,2)?></td>
					<!--발주수량-->
				...
				<?}elseif  ($loadPage== "po_cancel"){ //-------------- 판매자 송장 품목 취소(삭제)?>
					...
					<?}elseif ($loadPage == "30_16" ||$loadPage=="18R_21"){ //[판매자] 선적(30_16)/교환선적시 보여지는 내용?>
						...
						<?}else{?>
							...
							<td class="t-rt">
								...
								<?}else{?>
									<?=$odr_stock==0?"":number_format($odr_stock)?>
								<?}?>
							</td>
						<?}?>
					...
					?>
						...
						<?}else{?>
							...
							<td><?=$dc?></td>
							<td><?=$rhtype?></td>
							<?if($loadPage != "18R_19" && $loadPage!="19_1_05" && $loadPage!="19_1_06" && $loadPage!="30_14"){?><td class="t-rt"><?=$odr_stock==0?"":number_format($odr_stock)?></td><?}?>
						<?}?>
						<td class="t-rt">$<?=number_format($price,2)?></td>
						...
						<?if ($loadPage !="18R_06" && $loadPage !="18R_19" && $loadPage!="19_1_05" && $loadPage != "03_02"&& $loadPage != "04_01" && $loadPage!="19_06"){?>
						<td class="c-blue"><?
							if($loadPage!="05_04_1" || ($loadPage=="05_04_1" && $sell_mem_idx != $_SESSION["MEM_IDX"])){
					...
					<td colspan="12">
						<table class="detail-table">
							<tbody>
								...
								<tr class="noinput">
									<td colspan="2" lang="ko"><strong class="c-black" lang="en">Memo : </strong><?=$memo?> </td>
								</tr>
								<?}?>
				...
				<td colspan="9">
					<table class="detail-table">
						<tbody>
							...
							<tr>
								<td colspan="2" lang="ko"><strong class="c-black" lang="en">Memo : </strong><?=$row_ship["memo"]?></td>
							</tr>
							<?}?>
							<tr>
				...
				<tr>
					...
					<?}else{?>
						<th scope="col" lang="ko" class="t-orderoty">
							...
							<?}elseif($loadPage=="30_20_F" || $loadPage=="30_22_F"){
								$fault_select = get_any("odr_history", "fault_select", "odr_history_idx=$odr_history_idx");
								echo ($fault_select=="1")? "교환수량":"부족수량";
							?>
							<?}else{?>발주수량<?}?>
						</th>
						<?if($loadPage!="30_20_F" && $loadPage!="30_22_F"){?><th scope="col" lang="ko" class="t-supplyoty">공급수량</th><?}?>
						...
						<?}?>
					<?}?>
					<?if ($loadPage!="31_06" && $loadPage!="18R_06" && $loadPage!="04_01" && $loadPage!="03_02" && $loadPage!="30_15" && $loadPage!="18R_19" && $loadPage!="19_1_05" && $loadPage!="19_06"){?>
...
<?	
	...
	<script type="text/javascript">
		...
		//-->
			...
			$(".company-info-wrap select[name=com_type]").change(function(){
				...
				$("#dv_dosi").removeClass("type5");
				$("#dv_dosi").addClass("type6");
				$(".company-info-wrap input,select").attr("disabled",false);
				if($(".company-info-wrap select[name=nation]").val() == "1"){
					$(".company-info-wrap select[name=dosi], input[name=zipcode], input[name=sigungu]").attr("disabled",true);
					$("#dv_dosi").removeClass("type6");
					$("#dv_dosi").addClass("type5");
				}
			});
	...
	<input type="hidden" name="b_nation" id="b_nation" value="<?=$b_nation;?>">
	<table class="company-info-tb" style="width:745px">
		<tbody>
			<tr>
				<th scope="row"><strong class="c-red">*</strong> 회사구분</th>
				<td colspan="2">
					<div class="select type6 bd2" lang="ko" style="width: 215px;">
						<label class="c-blue"><?=($com_type)?GF_Common_GetSingleList("MEM",$com_type):""?></label>
						<?echo GF_Common_SetComboList("com_type", "MEM", "", 1, "True",  "", $com_type  , "lang='ko' id='must_field'");?>
					</div>
				</td>
			</tr>
			<tr>
				...
				<td colspan="2">
					<div class="select type5 bd2" id="dv_nation" lang="ko" style="width: 215px;">
						<label class="c-blue"><?=($b_nation)?GF_Common_GetSingleList("NA",$b_nation):""?></label>
						<?echo GF_Common_SetComboList("nation", "NA", "", 1, "True",  "", $b_nation  , "onchange='chgnation(this);' lang='en'");?>
					</div>
				</td>
			</tr>
			<tr class="indivi">
				<th scope="row"><strong class="c-red">*</strong> <span id="sp_com_nm">회사명</span></th>
				<td colspan="2"><input class="i-txt5 c-blue" maxlength="30" lang="ko" type="text" name="com_name" style="width:215px" value="<?=$com_name?>"></td>
			</tr>
			<tr>
				<th scope="row"><strong class="c-red">*</strong> <span id="sp_manager">담당자</span></th>
				<td colspan="2"><input class="i-txt5 c-blue" type="text" maxlength="30" lang="ko" name="manager" style="width:215px" value="<?=$manager?>"></td>
			</tr>
			<tr class="indivi">
				<th scope="row"><strong class="c-red">*</strong> <span id="sp_depart_nm">부서</span></th>
				<td colspan="2"><input class="i-txt5 c-blue" type="text" name="depart_nm" lang="ko" style="width:215px" value="<?=$depart_nm?>"></td>
			</tr>
			<tr class="indivi">
				<th scope="row"><strong class="c-red">*</strong> <span id="sp_pos_nm">직책</span></th>
				<td colspan="2"><input class="i-txt5 c-blue" type="text" maxlength="30" lang="ko" name="pos_nm" style="width:215px" value="<?=$pos_nm?>"></td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td colspan="2"><input class="i-txt5 c-blue" name="zipcode" type="text" maxlength="10" style="width:215px"  lang='en' value="<?=$zipcode?>"><span class="c-red">&nbsp;&nbsp;
					...
					<button type="button" onclick="openCommLayer('layer4','layerZipNew','?frm_name=f_<?=$loadPage;?>&frm_zip1=zipcode&frm_addr1=addr&frm_addr2=addr_det');" class="roadname" style="display:<?if ($nation!="1"){echo "none";}?>;"><img src="/kor/images/btn_roadname.gif" border=0 align=absmiddle alt="도로명주소"></button>
					</span></td>
			</tr>
			<tr>
				<th scope="row"><strong class="c-red">*</strong> 도/시</th>
				<td colspan="2">
					<div class="select type5 bd2" id="dv_dosi" lang='en' style="width: 215px;display:<?=($dosi!=""|| ($dosi=="" && $dositxt==""))?"":"none"?>;">
						<label class="c-blue"><?=($dosi)?GF_Common_GetSingleList_LANG("NA",$dosi,""):""?></label>
						<?=GF_Common_SetComboList("dosi", "NA", $nation, 2, "True",  "", $dosi , "onchange='chgdosi(this);' lang='ko'");?>
					</div>
			...
			<tr>
				...
				<td colspan="2"><input class="i-txt5 c-blue w50" name="addr_det" lang='en' value="<?=$addr?>" type="text"></td>
			</tr>
			<tr>
				<th scope="row"><strong class="c-red">*</strong>  주소</th>
				<td colspan="2" lang="en">
					<input type="hidden" class="i-txt3" placeholder="모국어" lang="ko" name="addr" id="addr" value="<?=$addr?>" style="width:308px"><br>
					<!--span class="c-blue" style="width:508px" id="sp_addr_en" lang="en"><u><?=$addr_en?></u></span-->
					<span class="c-blue"  id="sp_addr" style="width:508px" lang="ko"><u><?=$addr?></u></span>
				</td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td colspan="2">
					<input class="i-txt5 c-blue" name="homepage" type="text" style="width:215px" lang='en' value="<?=$homepage?>">
								...
								<table class="detail-table w100">
									<tbody>
										<tr>
											...
											<td colspan="3">
												<ul lang="en" class="list-type5">
													<li><label class="ipt-chk chk2 c-blue"><input type="checkbox"><span></span> UPS :  US $30.00</label></li>
													<li><label class="ipt-chk chk2 c-blue"><input type="checkbox"><span></span> UPS :  US $30.00</label></li>
													<li><label class="ipt-chk chk2 c-blue"><input type="checkbox"><span></span> UPS :  US $30.00</label></li>
												</ul>
											</td>
										...
										<tr>
											...
											<td><span class="c-grey2">운송회사</span> <span lang="en">DHL US $ 30.00</span></td>
											<th scope="row" class="w70p c-red"><span lang="en">Account No.</span></th>
											<td><input type="text" class="i-txt2 c-blue" value="123456789" style="width:92px"></td>
										</tr>
										<tr>
											<td colspan="4" lang="en"><strong class="c-red">Memo</strong> <input type="text" class="i-txt2 c-blue" value="XXXXXXXXXXXXXXXXXXXX" style="width:330px"></td>
										</tr>
										<tr>
								...
								<table class="detail-table w100">
									<tbody>
										<tr>
											...
											<td class="w180p">
												<span class="c-grey2">운송회사</span>
												<div class="select type4" lang="en" style="width:100px">
													<label class="c-blue">DHL</label>
													<select>
														<option>DHL</option>
														<option>UPS</option>
														<option>Fedex</option>
														<option>다른운송업체</option>
													</select>
												</div>
											</td>
											<th scope="row" class="w70p"><span lang="en">Account No.</span></th>
											<td><input type="text" class="i-txt2 c-blue" value="123456789" style="width:92px"></td>
										</tr>
										<tr>
