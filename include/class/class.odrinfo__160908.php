		...
		while($row = mysql_fetch_array($result)){
			...
			<tr id="tr_<?=$odr_det_idx?>"><!------------ 내용 첫 번째 tr -------------->
				...
				<? //-------------- 송장(30_08) : 판매자 페이지에서 보여지는 송장 목록 -------------------------------
					...
					<?}else{?>
					<td class="t-lt"><input type="text" class="i-txt4" name="part_no[]" value="<?=$part_no?>" maxlength="24" style="width:<?=($det_cnt>1)? "190":"210";?>px; ime-mode:disabled" ></td>
					<td class="t-lt"><input type="text" class="i-txt4" name="manufacturer[]" value="<?=$manufacturer?>" maxlength="20" style="width:<?=($det_cnt>1)? "156":"184";?>px; ime-mode:disabled" ></td>
					<td><input type="text" class="i-txt4" name="package[]" value="<?=$package?>" maxlength="10" style="width:75px; ime-mode:disabled" ></td>
					<td>
						<input type="text" name="rosh[]" maxlength="4" class="i-txt4 t-ct" style="width:35px; ime-mode:disabled" value="<?=$dc;?>">
					</td>
					<td>
					...
					<!--공급수량-->
					<td class="t-rt">
						<input type="text" name="supply_quantity[]" class="i-txt4 c-red2 onlynum numfmt t-rt" value="<?=($supply_quantity>0)? number_format($supply_quantity):""?>" maxlength="10" style="width:64px" origin_qty="<?=$origin_qty;?>">
					</td>
				...
				<?}elseif  ($loadPage== "po_cancel"){ //-------------- 판매자 송장 품목 취소, 구매자 송장(Invoice)받고 취소(삭제)?>
					<td>
					...
					<?}elseif ($loadPage == "30_16" ||$loadPage=="18R_21"){ //[판매자] 선적(30_16)/교환선적시 보여지는 내용?>
						...
						<?}else{?>
							<td class="t-rt">$<?=number_format($price,2)?></td>
							<td class="c-blue t-rt"><?=$odr_quantity==0?"":number_format($odr_quantity)?></td>
							<td class="c-red t-rt"><?=$supply_quantity==0?"":number_format($supply_quantity)?></td>
						<?}?>
						<?=($period)?"<td class='c-red'>".$period:(($part_type=="2"||$part_type=="5"||$part_type=="6")?"<td class='c-red'><span lang='ko'>확인</span>":"<td>Stock")?></td>
						<tr class="bg-none">
