		...
		while($row = mysql_fetch_array($result)){
			...
			<tr id="tr_<?=$odr_det_idx?>"><!------------ 내용 첫 번째 tr -------------->
				...
				<?}elseif ($loadPage== "31_05"){ //------------------------------------------------------------------------------------------------------?>
					...
					<td>
						<input type="text" class="i-txt2 c-blue onlynum numfmt t-rt" maxlength="10" name="odr_quantity[]" odr_det_idx="<?=$odr_det_idx?>" supply_quantity="<?=$supply_quantity;?>" quantity="<?=$quantity + $odr_quantity;?>" amd_yn="Y" value="<?=$supply_quantity==0?"":number_format($supply_quantity)?>" style="width:56px;">
					</td>
						...
						<td colspan="12" style="padding:0;">
							...
							<table class="detail-table" style="margin:0">
								<tbody>
									...
									<tr class="noinput">
										<th scope="row" style="width:70px">부품상태&nbsp: </th>
										<td><span >&nbsp<?=GF_Common_GetSingleList("PARTCOND",$part_condition)?></span></td>
										<th scope="row" style="width:70px">포장상태&nbsp: </th>
										<td><span >&nbsp<?=GF_Common_GetSingleList("PACKCOND1",$pack_condition1)?> / <?=GF_Common_GetSingleList("PACKCOND2",$pack_condition2)?> </span></td>
									</tr>
									<?}?>
				...
				<? //-------------- 송장(30_08) : 판매자 페이지에서 보여지는 송장 목록 -------------------------------
					...
					<!--공급수량-->
					<td class="t-rt">
						<input type="text" name="supply_quantity[]" class="i-txt4 c-red2 onlynum numfmt t-rt" value="<?=($supply_quantity>0)? number_format($supply_quantity):""?>" maxlength="10" style="width:58px" origin_qty="<?=$origin_qty;?>">
					</td>
						...
						<td colspan="11" style="padding:0">
							<table class="detail-table">
								<tbody>
									...
									<tr>
										<td colspan="2" ><strong class="c-black">&nbsp;Memo&nbsp;&nbsp;</strong> <input type="text" class="i-txt5" name="memo[]" value="" style="width:415px"></td>
									</tr>
								</tbody>
							</table>
						</td>
				<?}elseif  ($loadPage== "po_cancel"){ //-------------- 판매자 송장 품목 취소, 구매자 송장(Invoice)받고 취소(삭제)?>
					...
					<?}?>
						...
						<tr>
							...
							<td colspan="10" class="img-cntrl-list" style="text-align:left;">
								...
								<?}?>
								<span style="font-size:11px;"><strong class="c-red">라벨/부품 사진을 첨부해 주십시오. 분쟁 발생 시 보호받을 수 있습니다.</strong></span>
							</td>							
						</tr>
					...
					?>
						...
						<input type="hidden" name="odr_det_idx" value="<?=$odr_det_idx?>"/>
							...
							<td><?=$dc?></td>
							<td><?=$rhtype?></td>
							<?if($loadPage != "18R_19" && $loadPage!="19_1_05" && $loadPage!="19_1_06" && $loadPage!="30_14"){?><td class="t-rt"><?=$supply_quantity==0?"-":number_format($supply_quantity)?></td><?}?>
						<?}?>
						<td class="t-rt">$<?=number_format($price,2)?></td>
						...
						<?if ($loadPage !="21_04" || $loadPage !="30_20" || $loadPage =="30_14" ){?>
								<?if($loadPage!="02_02"&& $loadPage !="05_04_1" && $loadPage !="30_20" && $loadPage !="30_14"){?><td class="t-rt c-red"><?=$supply_quantity==0?"":number_format($supply_quantity)?></td><?}?>
						...
						<td class="c-blue"><?
							if($loadPage!="05_04_1" || ($loadPage=="05_04_1" && $sell_mem_idx != $_SESSION["MEM_IDX"])){
						?><a href="javascript:layer_company_det(<?=$com_idx?>);" style="color:#00759e;"><?=cut_len($company_nm,10,".");?></a><?
							}?></td><?
						}?>
					</tr>
					...
					<td colspan="12" style="padding:0;">
						<table class="detail-table" style="margin:0;">
							<tbody>
								...
								<tr class="noinput">
									<td colspan="2" ><strong class="c-black">사유 : </strong><?=$det_reason;?> </td>
								</tr>
								<?}?>
								<?if ( $loadPage =="21_04" ){?>
								<tr>
...
<?
	...
	while($row = mysql_fetch_array($result)){
			...
			if ($loadPage!="12_07"){ //수정발주서 Sheet(Purchase Order Amendment)
				$extra = "";
					...
					<tr>
						...
						<td class="t-rt">
							...
							if ($loadPage!="18_2_09" && !($loadPage=="30_09" && $part_type=="2" && $pay_cnt<2) ){
								echo "$".number_format($odr_quantity*$price,2);
							}
							?>
						</td>
					</tr>
...
if ($for_readonly != "P") {?>
	<ul class="total-price">
		...
		{	
			...
			$vat_plus =  number_format($tot*($vat_price/100));
			$tot = $tot + $vat_plus;			
		}
		...
		?>			
			...
			{	
			?>
				<li class="sub"><strong>Sub Total :</strong><span id="sub_total">$<?=number_format($tot_vat_minus,2)?></span></li>	
				<li class="sub"><strong><?=$tax_name?> :</strong><span>$<?=number_format($vat_plus,2)?></span></li>		
			<?
			}
			else
			{
		...
		?>
			<ul class="total-price-ko">						
				<li class="total"><strong>Total :</strong><span id="g_total">￦<?=(number_format($won_change*$tot))?></span></li>	
			</ul>
		<?
		}
		?>
			...
			while($row = mysql_fetch_array($result)){
				...
				?>
					<li <?=$cls?>>
						...
						<?}else{?>
							<span class="etc"><span ><?if ($etc2){echo openSheet($status, $etc2,$odr_idx);}?></span></span>
						<?}?>
						<?}else{?>
							<span class="etc"><span ><?if ($etc1){echo openSheet($status, $etc1,$odr_idx);}?></span></span>
			...
			<tbody>
				<tr><?
					...
					<td class="company" style="width:33%">
						<img src="/kor/images/nation_title_<?=$buy_com_nation?>.png" alt="<?=GF_Common_GetSingleList("NA",$buy_com_nation)?>"> &nbsp&nbsp<span class="name"><?=$buy_com_name?></span>
					</td>
					...
					<?	  
						   ...
						   break;
						  case "09_03":?>
					<td class="company" style="width:33%;"><img src="/kor/images/nation_title_<?=$buy_com_nation?>.png" alt="<?=GF_Common_GetSingleList("NA",$buy_com_nation)?>"> <span class="name"><?=$buy_com_name?></span></td>
					...
					if($odr_his[charge_ty] == "D"){
						echo "계약금 (<strong class='c-blue'><span lang='en'>$".$pay; 
						...
						echo "총 금액 (<strong class='c-blue'><span lang='en'>".$pay; 
					...
					<?if ($odr_idx>0){
						echo "부대비용 (<strong class='c-blue'><span lang='en'>$".$pay;
					}else{
					...
					<?if ($ship_info!="6") {?>
						...
						<?}else{?>
							<img src="/kor/images/icon_<?=strtolower(GF_Common_GetSingleList("DLVR",$ship_info))?>.gif" height="15" alt="" style="margin-bottom:2px;">
						<?}?>
						<?if ($ship_info!="6") {?>
								...
								<table style="border:0; padding:5px;">
									<tr>
										...
										<td>
											<!-- 운송 기타정보-->
											<table>
												<?if($ship[insur_yn] == "o"){?><tr><td colspan="2">운송보험 : <span class="c-red">Yes</span></td></tr><?}?>
												<?if($ship[delivery_addr_idx] > 0){?>
			...
			<tr class="bg-none">
				...
				<td colspan="9" style="padding:0">
					<table class="detail-table">
						<tbody>
							<tr>
								<th scope="row" style="width:200px;padding-top:7px;">
									선적정보 : 
									<?
		...
		<table class="stock-list-table" id="list_<?=$loadPage?>">
			<thead>
				<tr>
					...
					<?
					if ($loadPage=="21_04" || $loadPage=="30_15" ){
						$part_no_style = "style='width:280px;'";
					}
							...
							<div class="check-wrap">
								<table class="detail-table w100" style="margin:0;">
									<tbody>
										...
										<tr>
											<td colspan="4" ><strong class="c-red">Memo</strong> <input type="text" class="i-txt2 c-blue" value="XXXXXXXXXXXXXXXXXXXX" style="width:330px"></td>
										</tr>
										<tr>
											<td colspan="4"><label class="ipt-chk chk2"><input type="checkbox" <?if ($ship_info=="6") {echo "disabled";}?> name="delivery_chg" id="delivery_chg"  <?if ($delivery_addr_idx){echo "checked class='checked'";}?>><span></span> 배송지 변경</label></td>
										</tr>
									</tbody>
								</table>
							</div>
								...
								<table class="detail-table w100">
									<tbody>
										<tr>
											...
											<th scope="row" class="w70p"><span lang="en"><font color='black'>Account No.</font></span></th>
											<td><input type="text" class="i-txt2 c-blue" value="123456789" style="width:92px"></td>
										</tr>
										<tr>
											<td colspan="4" ><strong class="c-red">Memo</strong> <input type="text" class="i-txt2 c-blue" value="XXXXXXXXXXXXXXXXXXXX" style="width:323px"></td>
										</tr>
										<tr>
