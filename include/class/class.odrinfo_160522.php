		...
		while($row = mysql_fetch_array($result)){
			...
			<tr id="tr_<?=$odr_det_idx?>"><!------------ 내용 첫 번째 tr -------------->
				<?if ($loadPage== "30_06"|| $loadPage== "09_03" || $loadPage== "30_22" || $loadPage== "31_04" || $loadPage== "01_29" ){ //판매자 페이지에서 보여지는 내용(30_06 layer)?>
					...
					<?}?>
				<?}elseif ($loadPage== "30_20_F" || $loadPage== "30_21_F"){ //--------------------------- 선적(Fault), 수령(fault)-POP -----------------------------------------------------------
				...
				?>
					...
					<td class="c-blue"><a href="javascript:layer_company_det(<?=$com_idx?>);"><?=cut_len($company_nm,10,".");?></a></td>
					<?if($loadPage == "30_20_F"){?>
					<!-- 복사해온거 시작-->
					</tr>
					<tr class="bg-none">
				...
				<?}elseif  ($loadPage== "po_cancel"){ //-------------- 판매자 송장 품목 취소(삭제)?>
					...
					?>
						...
						<?}else{?>
							...
							<td><?=$dc?></td>
							<td><?=$rhtype?></td>
							<?if($loadPage != "18R_19" && $loadPage!="19_1_05" && $loadPage!="19_1_06" && $loadPage!="30_14"){?><td class="t-rt"><?=$quantity==0?"":number_format($quantity)?></td><?}?>
						<?}?>
						<td class="t-rt">$<?=number_format($price,2)?></td>
						...
						<!-- 발주수량/반품수량-->
							...
							<?}elseif($loadPage == "18R_19" || $loadPage=="19_1_05" || $loadPage=="19_1_06" || $loadPage=="30_14"){
						...
						<?if ($loadPage !="18R_06" && $loadPage !="18R_19" && $loadPage!="19_1_05" && $loadPage != "03_02"&& $loadPage != "04_01"){?>
						<td class="c-blue"><?
							if($loadPage!="05_04_1" || ($loadPage=="05_04_1" && $sell_mem_idx != $_SESSION["MEM_IDX"])){
					...
					<td colspan="12">
						<table class="detail-table">
							<tbody>
								...
								<?}?>
								<?if ($loadPage != "03_02" && $loadPage!="05_04_1" && $loadPage!="13_04s" && $loadPage!="13_02s" && $loadPage != "19_1_06"){?>
								<tr class="noinput">
	...
	while($row = mysql_fetch_array($result)){
			...
			?>
				...
				<?}else{ //---------- 공통(턴키 아니고, 30_17 아닌것) ------------?>
					<tr>
						<td><?=$i?></td>
						<td class="t-lt"><?=$part_no?>/fault_quantity:<?=$fault_quantity;?></td>
						<td class="t-lt"><?=$manufacturer?>, <?=$package?>, <?=$dc?>, <?=$rhtype?><?=$extra?></td>
						<td class="t-rt"><?=$odr_quantity==0?"":number_format($odr_quantity)?></td>
						<td class="t-rt">$<?=number_format($price,2)?></td>
						<td><?=($period)?( QRY_CNT("odr_history", "and  odr_idx = $odr_idx and status = 19 ")>0?"Stock":$period):(($part_type=="2"||$part_type=="5"||$part_type=="6")?"<span lang='ko' class='c-red'>확인</span>":"Stock")?></td>
...
<!-- layer-step(거래단계 표기) -->
	...
	<?if ($loadPage!="31_04" && $loadPage!="19_1_05"){ //납기확인 전까지는 history 필요없댐, 환불완료 메시지창?>
		...
		<table>
			<tbody>
				<tr><?
					...
					<td><?if ($fault_select){?><div class="re-select"><strong>선택</strong><?if ($fault_select=="1"){?><em>교환</em><?}else{?><em>반품</em><?}?></div><?}?></td>
					<td class="c-red2 w100 t-ct">
						<?= "부대비용 (<strong class='c-blue'><span lang='en'>US$".$pay;?></span></strong>) 결제가 완료되었습니다.
					</td>
					</tr></tbody></table></div>	
					<?
							echo layerInvListData($loadPage ,$odr_idx, $odr_det_idx , $odr_history_idx);
					...
					?>
						<td colspan="2" class="c-red2 t-rt">운송회사 : <img src="/kor/images/icon_<?=strtolower(GF_Common_GetSingleList("DLVR",$ship_info))?>.gif" height="15" alt=""> &nbsp;&nbsp;&nbsp;운송장번호 : <span lang="en" class="c-blue"><?=$delivery_no?></span></td>
					</tr>
					<tr>
						<td class="c-red2"><?if ($fault_select){?><div class="re-select"><strong>선택</strong><?if ($fault_select=="1"){?><em>교환</em><?}else{?><em>반품</em><?}?></div><?}?></td>
						<td class="c-red2 t-rt">선적서류 <span lang="en"><!--Download--></span> : <a href="#" class="btn-view-sheet-3011" for_readonly="Y"><img src="/kor/images/btn_commercial_invoice.gif" alt="Commercial Invoice"></a> <a href="#"  class="btn-view-sheet-3011" for_readonly="P"><img src="/kor/images/btn_packing_list.gif" alt="Packing List"></a></td>
						</tr></tbody></table></div>	
						<?
							   echo layerInvListData($loadPage ,$odr_idx, $odr_det_idx, $odr_history_idx);
	...
	<?}elseif ($loadPage =="30_22"){ //------------------------- 수령(판매자) 버튼 ----------------------------------------?>
		<div class="btn-area t-rt">
			<a href="#" class="btn_3022"  odr_idx="<?=$odr_idx?>"><img src="/kor/images/btn_deposit3.gif" alt="완료"></a>
		</div>
...
//---------------------- GET_ODR_DET_LIST, history 아래 버튼들---------------------------------------------------------------------------------------------------------------
		...
		<table class="stock-list-table" id="list_<?=$loadPage?>">
			<thead>
				<tr>
					...
					<th scope="col" class="t-dc">D/C</th>
					<th scope="col" class="t-rohs">RoHS</th>
					<?if($loadPage!="18R_19" && $loadPage!="19_1_05" && $loadPage!="19_1_06" && $loadPage != "30_14" && $loadPage != "30_20_F"){?><th scope="col" class="t-oty">O'ty</th><?}?>
					<th scope="col" class="t-unitprice">Unit Price</th>
					<?if ($loadPage=="21_04"){?>
					...
					<?}else{?>
						...
						<?}?>
					<?}?>
					<?if ($loadPage!="31_06" && $loadPage!="18R_06"&& $loadPage!="04_01" && $loadPage!="03_02" && $loadPage!="30_15" && $loadPage!="18R_19" && $loadPage!="19_1_05"){?>
	...
	<?if ($loadPage =="18R_08" || $loadPage =="18R_06" || $loadPage =="18R_16" || $loadPage =="18R_19" || $loadPage =="30_14" || $loadPage =="30_20_F"){  //--------------- 거절(18R_08) 메시지 내역(주고, 받고) -----------------------------------------------------------------------------------------------
	?>
	<div class="layer-data">
		<table class="stock-list-table">
			<tr class="bg-none">
