			  ...
			  global $file_path;
			   ...
			   $memo = $fty_his[reason];
				...
				case "21_5_13":
						...
						<?}elseif ($loadPage =="21_1_14"){//--------------->결제완료
								if($reason_ty=='6'){//-->연구소의뢰?>
									<td class="c-red2 w100 t-ct">판매자의 부대비용, 재 작업 비용 (<span class="c-blue" lang="en">US$<?=$pay?></span>) 결제가 완료되었습니다.</td>
								<?}else{?>
	...
	<div class="layer-data">
		<table class="stock-list-table">
			...
			<tbody>
				...
				while($row = mysql_fetch_array($result)){
					$i++;
					$fty_history_idx = $row["fty_history_idx"];
					$status = $row["status"];
						...
						<table Style="width:100%;">
							<tr>
								<td class="t-lt" lang="ko">
									...
									<span style="cursor:pointer;" OnClick="show_msg(<?=$fty_history_idx;?>);"><?=$reason_title?></span>
								</td>
								<td class="t-rt" lang="en" Style="width:100px; padding-right:5px;"><?=date("j F Y");?></td>
							</tr>
							<tr id="msg_cont_<?=$fty_history_idx;?>" Style="display:none;">
...
?>
	<?if($freight>0){?>
	<tr><td><?=$i++;?></td><td colspan="2" class="t-lt"> Freight </td><td colspan="3">&nbsp; </td><td class="t-rt">$<?=number_format($freight,2)?></td></tr>
	<?}?>
	<tr><td><?=$i++;?></td><td colspan="2" class="t-lt"> Rework Fee 20% </td><td colspan="3">&nbsp; </td><td class="t-rt">$<?=number_format($reworkfee,2)?>  </td></tr>
	<tr><td><?=$i++;?></td><td colspan="2" class="t-lt"> Test Fee </td><td colspan="3">&nbsp; </td><td class="t-rt">$<?=number_format($testfee,2)?> </td></tr>	
<?	}else{	//buyer ----------------------------------<<
	$reworkfee = $tot * 0.2;
	$testfee = 1000;
	$tot =$reworkfee + $testfee;
?>
	<tr><td><?=$i+2?></td><td colspan="2"> Compensation Fee 20%  </td><td colspan="3">&nbsp; </td><td class="t-rt">$<?=number_format($reworkfee,2)?>  </td></tr>
