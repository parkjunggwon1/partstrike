...
function GET_RCD_DET_LIST($part_type, $odr_type, $searchand ,$fr){
	...
	while($row2 = mysql_fetch_array($result2)){
		...
		<tr class="criteria" criteria_idx="<?=$criteria_now_idx?>">
			...
			<?if ($odr_status==2 || $odr_status==18 || $odr_status==31){?>
...
<?	
	...
	for ($i = $end; $i >= $start; $i--) {  
		...
		$searchand = "and odr_status IN('13','14','15','25','26','27','28','29','30') and DATE_FORMAT(reg_date,'%Y-%m') = '$yr-$mki' and odr_no <> '' and invoice_no <> '' and ".($odr_type=="S"?"sell_":"")."mem_idx =$this_mem_idx $sn";
			...
			while($row = mysql_fetch_array($result)){			//$result : odr
				...
				while($row2 = mysql_fetch_array($result2)){
					...
					if ($com_idx ){
						...
						<?if ($odr_type == "B"){?><td class="txt-r"><?if ($end_yn=="Y"){?><div class="c-blue company_div" style="cursor:pointer;" onclick="side_company_info2(<?=$com_idx?>,'<?=$odr_type?>')"><?=$company_nm?></div>
						<?
						$now_date = date("Y-m-d H:i:s", strtotime(date('Y-m-d').' - 6month')); 
						$reg_date = get_any("odr","reg_date" , "odr_idx = $odr_idx");
						$fault_valid_yn = $now_date<=$reg_date?"Y":"N";					
