...
function GET_RCD_DET_LIST($part_type, $odr_type, $searchand ,$fr){
	...
	$result2 =QRY_RCD_DET_LIST(10,$searchand,1,"a.odr_idx desc, b.part_type ASC, b.odr_det_idx ASC"); //오름차순 정렬
	while($row2 = mysql_fetch_array($result2)){
		...
		if (($status && $chk) || ($odr_type =="B" && $save_yn =="Y")) {
			...
			if ($criteria_now_idx != $criteria_idx) {
				if ($bgcolor == "") { 
					...
					$bgcolor_now ="background-color:#ffe655;"; //2016-04-10 노란색
				}
			}
		...
		<tr>
			<td colspan="<?=$colspan?>" class="title-box" >
				<?if ($odr_type == "S" && ($criteria_now_idx != $criteria_idx)){?><div class="nation2" lang="en"><img src="/kor/images/nation_title_<?=$buy_nation?>.png" alt="<?=$buy_company_nm?>">&nbsp;&nbsp;<a href="javascript:side_company_info(<?=$buy_com_idx?>)"><?=($status==1 || $status==16 || $status==7)? "&nbsp;":$buy_company_nm;?></a></div><?}?>
		...
		<?if($fr=="M"){
				...
				if ($qrycnt >0) { 					
					...
					$status_now = $status;
					$goJump = "style='cursor:pointer;' onclick=\"javascript:goMenuJump('".$status.":".$sell_mem_idx.":odr:Y:".$page."')\" ";
				}else{
					if ($odr_type =="B" && $save_yn =="Y"){
						$goJump = "title=\"저장\" style='cursor:pointer;' onclick=\"javascript:openCommLayer('layer3','05_04', '?odr_idx=".$odr_idx."')\" ";	
					}else{
		...
		<tr class="criteria" criteria_idx="<?=$criteria_now_idx?>">
			...
			<td class="t-lt" <?=$goJump?>><?=get_cut($part_no,($odr_type == "B"?"15":"30"),($fr=="S"?"":"."))?></td>
			<td class="t-lt" <?=$goJump?>>
			<?if ($fr=="M"){
				echo get_cut($manufacturer,($odr_type == "B"?"10":"15"),".");
			}else{
				echo get_cut($manufacturer,($odr_type == "B"?"10":"20"),".");				
			}?></td>
			<td <?=$goJump?>><?=$package;?></td>
			<td <?=$goJump?>><?=$dc;?></td>
			<td <?=$goJump?>><?=$rhtype?></td>
			<td  <?=$goJump?> class="t-rt"><?=$quantity==0?"":number_format($quantity)?></td>
			<td  <?=$goJump?> class="t-rt">$<?=number_format($price,2)?></td>
			<?if ($fr == "S"){?>
			<td  <?=$goJump?>><?=$odr_quantity==0?"":$odr_quantity?></td>
			<td  <?=$goJump?>><?=$supply_quantity==0?"":$supply_quantity?></td>
			<?}?>
			...
			<?if ($odr_type == "B") {  //--구매자 화면일경우?>
				...
				<td class="c-blue">&nbsp;</td>
				<?}else{?>
				<td class="c-blue" onclick='side_company_info(<?=$com_idx?>)' Style="cursor:pointer;"><?=get_cut($company_nm,12,".") //상호 노출?></td>
				<?}?>
			<?}?>
		</tr>
	...
	<table class="stock-list-table">
		<thead>
			<tr>
				...
				<?if ($odr_type == "B"){?><th scope="col" style="width:77px">Nation</th><?}?>
				<th scope="col" class="t-lt">Part No.</th>
				<th scope="col" class="t-lt" style="width:<? echo ($odr_type == "S") ? '67px' : '53px' ?>">Manufacturer</th>
				...
				<?if ($odr_type == "B"){?><th scope="col" style="width:38px">Company</th><?}?>
			</tr>
		</thead>
		<tbody id="orderlist">
		<?	//for ($i = 1; $i<=6; $i++){
...
function GF_GET_REMIT_LIST($yr,$mon,$remit_ty){
	...
	**/
	...
	//echo $sql;	?>	<table class="stock-list-table">
			<thead>
				...
				<th scope="col" class="th2 t-rt" lang="ko">금액</th>
	...
	if ($remit_cnt > 0 ) {
		...
		<tr  style="background-color:<?=$bgcolor?>;">
			...
			<td class="t-lt" lang="ko"><?=$mem_nm?></td>
				...
				<td class="c-purple"><?=$invoice_no?></td>
				<?if ($charge_type =="1"){?>
					<td class="c-blue2 t-rt">$
				<?}else{?>
					<td class="c-red2 t-rt">$
				<?}?><?=number_format($charge_amt,2)?></td>
				<td class="c-blue2 t-rt"><?=number_format($mybank_amt,2)?></td>
...
function GF_GET_TURNKEY_RCD_LIST($odr_type, $sch_part_no,$yr,$mon,$this_mem_idx){
			...
			while($row = mysql_fetch_array($result)){			
				...
				while($row2 = mysql_fetch_array($result2)){
					...
					<tr>
						...
						<?if ($odr_type=="S"){?>						
							<div class="mr-tb5"><a href="javascript:side_company_info(<?=$buy_com_idx?>)"><img src="/kor/images/nation_title_<?=$buy_nation?>.png"> <span class="c-blue" lang="en"><?=$buy_company_nm?></span></a><a href="javascript:;" class="f-rt recordbt"><img src="/kor/images/btn_record.gif" alt="기록"></a></div>
							<h3 class="title"><img src="/kor/images/stock_title0<?=$part_type?>.gif" alt="<?=GF_Common_GetSingleList("PART",$part_type)?>"></h3>
						</td>
						<?}else{?>
							<h3 class="title"><img src="/kor/images/stock_title0<?=$part_type?>.gif" alt="<?=GF_Common_GetSingleList("PART",$part_type)?>"></h3><a href="javascript:;" class="f-rt recordbt"><img src="/kor/images/btn_record.gif" alt="기록"></a>
						<?}?>
						</td>
					</tr>
					<tr>
					...
					<?if ($odr_type == "B"){?><td><?if ($end_yn=="Y"){?><div class="c-blue" style="cursor:pointer;" onclick="side_company_info(<?=$com_idx?>)"><?=$company_nm?></div>
					<?
...
<?}
function GF_GET_RECORD_LIST($odr_type, $sch_part_no,$yr,$mon,$this_mem_idx){
	$colspan = $odr_type == "B"? "11" : "9";
	$bgcolor="#f7f7f7";
	...
	<table class="stock-list-table bg-type4">
		<thead>
			<tr>
				...
				<th scope="col" class="t-rt" style="width:65px">O'ty</th>
				<th scope="col" class="t-rt" style="width:65px">Unit Price</th>
				<th scope="col" style="width:65px">Amount</th>
				<?if ($odr_type == "B"){?><th scope="col">Company</th><?}?>
			</tr>
		</thead>
<?	
	...
	$conn = dbconn();	
	$k=1;
	for ($j = $start_yr; $j <= $end_yr; $j++) {   // $j : year, $i : month
	for ($i = $start; $i <= $end; $i++) {  
		$mki = substr("0$i" ,-2);
		$yr = $j;
		if ($sch_part_no){
		...
		$sql = "SELECT * FROM odr where  odr_status <> 99 and DATE_FORMAT(reg_date,'%Y-%m') = '$yr-$mki' and odr_no <> '' and invoice_no <> '' and ".($odr_type=="S"?"sell_":"")."mem_idx =$this_mem_idx $sn";
		//echo $sql;
		$result=mysql_query($sql,$conn) or die ("SQL ERROR : ".mysql_error());
		$odr_cnt=mysql_num_rows($result);
		if ($odr_cnt>0) {?>
			...
			while($row = mysql_fetch_array($result)){			//$result : odr
				...
				while($row2 = mysql_fetch_array($result2)){
					...
					if ($com_idx ){
						...
						<td>$<?=number_format($odr_quantity*$price,2)?></td>
						<?if ($odr_type == "B"){?><td><?if ($end_yn=="Y"){?><div class="c-blue" style="cursor:pointer;" onclick="side_company_info(<?=$com_idx?>)"><?=$company_nm?></div>
						<?
						$now_date = date("Y-m-d H:i:s", strtotime(date('Y-m-d').' - 6month')); 
						$reg_date = get_any("odr","reg_date" , "odr_idx = $odr_idx");
	...
	?>
		<div class="layer-step" lang="ko" style="display:none;">
			...
			while($row = mysql_fetch_array($result)){
				...
				if ($session_mem_idx != $reg_mem_idx){$cls = "class='c1".$blank."'";}
			  ...
			  global $file_path;
			   ...
			   $memo = $fty_his[reason];
				...
				?>		
							...
							<?if ($loadPage=="21_1_02"){?><td class="c-red2 w100 t-ct">동의서가 도착했습니다.</td><?}?>
								<td class="file-memo"><?if ($memo){?>Memo: <strong><?=$memo?><?}?></strong></td>
							</tr>
							</tbody></table></div>
				...
				<?		echo layerListData($loadPage ,$odr_idx,$odr_det_idx,$his_ty,$odr_history_idx);
					break;
				case "21_1_07":?>
						<td class="file"><span lang="ko"><?echo $return_method == "1" ? "반품포기" : "선적정보"?></span></td>
						<td class="file-memo">Memo: <strong><?=$memo?></strong></td>
					</tr>
					</tbody></table></div>
...
function layerListData($loadPage ,$odr_idx,$odr_det_idx ,$his_ty,$fty_history_idx=""){
	...
	<div class="layer-data">
		...
		<table class="stock-list-table" id="list_<?=$loadPage?>">
			...
			<?if ($loadPage =="21_07" || $loadPage=="21_1_02"|| $loadPage=="21_1_07"|| $loadPage=="21_1_14"|| $loadPage=="21_2_03"|| $loadPage=="21_6_03"|| $loadPage=="21_5_12"|| $loadPage=="21_5_13"){?><td class="c-blue"><?=$company_nm?></td><?}?>
			</tr>
			<?if ($loadPage!= "21_2_10"){?>
			<tr class="bg-none">
					<td></td>
...
?>
	...
	<?}elseif($loadPage=="21_4_10" || $loadPage=="21_5_12"){?>
	...
	<?}elseif ($loadPage=="21_1_14"){
		...
		$rel_idx = get_any("member", "rel_idx" ,"mem_idx = $mem_idx");
		if ($reason_ty =="6") { 
			//내가 결제를 한적이 있으면 "반품방법", 안했으면 "결제"를 표시
				if (QRY_CNT("fty_history" , "and status = 27 and reg_mem_idx = $session_mem_idx") > 0){			
...
<?}elseif ($loadPage=="21_4_06"){
	//bankingchg 계산법이 어떻게 되는지? 리스트에 bankingchg가 있는데, 왜 subtot 에다가 bankingchg를 또 더하는지? 
	...
	$reworkfee = $tot * 0.15;
	$testfee = 1000;
	$tot = $tot + $escrow + $freight + $bankingchg + $reworkfee + $testfee;
?>
	<tr><td><?=$i+1?></td><td colspan="2" class="t-lt"> Escrow Fee </td><td colspan="3">&nbsp; </td><td class="t-rt">$<?=number_format($escrow,2)?></td></tr>
	<tr><td><?=$i+2?></td><td colspan="2" class="t-lt"> Freight </td><td colspan="3">&nbsp; </td><td class="t-rt">$<?=number_format($freight,2)?></td></tr>
	<tr><td><?=$i+3?></td><td colspan="2" class="t-lt"> Banking Charge  </td><td colspan="3">&nbsp; </td><td class="t-rt">$<?=number_format($bankingchg,2)?></td></tr>
	<tr><td><?=$i+4?></td><td colspan="2" class="t-lt"> Rework Fee 15% </td><td colspan="3">&nbsp; </td><td class="t-rt">$<?=number_format($reworkfee,2)?>  </td></tr>
	<tr><td><?=$i+5?></td><td colspan="2" class="t-lt"> Test Fee </td><td colspan="3">&nbsp; </td><td class="t-rt">$<?=number_format($testfee,2)?> </td></tr>	
<?	}else{
	$bankingchg = 30;
	$reworkfee = $tot * 0.15;
	$testfee = 1000;
	$tot =$escrow + $freight + $bankingchg + $reworkfee + $testfee;
?>
	<tr><td><?=$i+1?></td><td colspan="2"> Banking Charge  </td><td colspan="3">&nbsp; </td><td class="t-rt">$<?=number_format($bankingchg,2)?></td></tr>
	<tr><td><?=$i+2?></td><td colspan="2"> Compensation Fee 15%  </td><td colspan="3">&nbsp; </td><td class="t-rt">$<?=number_format($reworkfee,2)?>  </td></tr>
	<tr><td><?=$i+3?></td><td colspan="2"> Test Fee </td><td colspan="3">&nbsp; </td><td class="t-rt">$<?=number_format($testfee,2)?> </td></tr>	
<?
	}
}elseif($loadPage =="21_4_01"){//Request R&D Center
$escrow = $tot >=3000 ? $tot *0.01 : "30";
$freight = $odr[faulty_delivery_fee];
$bankingchg = 30;
$reworkfee = $tot * 0.15;
...
?>
	...
	Buyer must send the entire quantity to PARTStrike when it should be tested. Shipping fee will paid by PARTStrike for the moment after the result is being reported, PARTStrike will subtract the same amount with shipping fee from the responsible one's deposit. <br><br>
 
	Members agrees to follow the result of the Test Report from PARTStrike for the above items.</td><td colspan="1">&nbsp;</td></tr>
<?}
?>
</tbody>
	</table>	
<?	if($loadPage !="21_4_01"){?>
	<ul class="total-price">
		...
		<?if($loadPage =="21_4_06"){?>
			$<?=number_format($tot - $bankingchg,2)?>		
			<li class="sub"><strong>Banking Charge :</strong><span>$<?=number_format($bankingchg,2)?>		
		<?}else{?>
			$<?=number_format($tot,2)?>		
		<?}?>
