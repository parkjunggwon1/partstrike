		...
		while($row = mysql_fetch_array($result)){
			...
			<tr id="tr_<?=$odr_det_idx?>"><!------------ 내용 첫 번째 tr -------------->
				...
				<?}elseif ($loadPage== "31_05"){?>
					...
					<?//-----------------------------------------------------------------------------------------------------------------------------------------------
				}elseif ($loadPage== "05_04" ||$loadPage== "09_01" || $loadPage=="04_01"){  //구매자 페이지에서 보여지는 내용 (05_04 layer)?>
					<!--05_04----------------->
					<td>
						<label class="ipt-chk chk2">
							<input type="<?=($det_cnt>1)? "checkbox":"hidden";?>" name="odr_det_idx[]" amend_yn="<?=$amend_yn?>" class="<?=($part_type=="2" && $period*1> 2 && QRY_CNT("odr_history", "and  odr_idx = $odr_idx and status = 19 ")<=0) ? "endure":"stock"?>" value="<?=$odr_det_idx?>" <?if(($part_type=="2"||$part_type=="5"||$part_type=="6") && $period ==""){?>disabled<?}?> part_type="<?=$part_type?>"><span></span>
						</label>
					</td>
					<td><?=$i?></td>
					...
					<td><input type="text" class="i-txt2 c-blue  onlynum numfmt t-rt" maxlength="10" name="odr_quantity[]" odr_det_idx="<?=$odr_det_idx?>" value="<?=$odr_quantity==0?"":number_format($odr_quantity)?>" style="width:56px;" <?if ($part_type=="2"||$part_type=="5"||$part_type=="6"){?>readonly<?}?>></td>
					...
					<?if ($loadPage =="05_04" || $loadPage =="09_01"){?>
					<td class="c-red pd-0">
				...
				<?}elseif  ($loadPage== "30_08"){ //판매자 페이지에서 보여지는 송장 내용?>
					<td><?=$i?><input type="hidden" name="odr_det_idx[]" value="<?=$odr_det_idx?>"></td>
					<?if ($part_type=="7"){?>
					...
					<tr>
						...
						<!--------------------------------------------------------------------------------------------------------------------------------------------------->
					<?}elseif ($loadPage == "30_16" ||$loadPage=="18R_21"){ //판매자 페이지에서 선적/교환선적시 보여지는 내용?>
						<td><?=$i?></td>
						...
						<!------------------------------------------------------------------------------------------------------------------------------------------>
					<?}elseif  ($loadPage== "05_04_1" ||$loadPage== "30_10" || $loadPage== "30_15"  || $loadPage== "30_20" || $loadPage=="31_06"|| $loadPage=="01_37" ||$loadPage == "21_04" ||$loadPage == "18R_05" ||$loadPage == "18R_06" ||$loadPage =="18R_08" ||$loadPage =="18R_16"||$loadPage =="18R_19"||$loadPage =="04_01"||$loadPage =="19_06"||$loadPage =="19_08"||$loadPage =="19_1_06" || $loadPage =="08_02" || $loadPage =="10_02" ||$loadPage=="10_04" ||$loadPage =="13_02" || $loadPage =="13_04"|| $loadPage =="02_02" || $loadPage =="03_02"|| $loadPage== "30_23" || $loadPage =="18_1_04"){ //구매자 페이지에서 보여지는 송장 내용
						$com_idx = $rel_idx==0 ? $sell_mem_idx : $rel_idx;
						$company_nm = get_any("member","mem_nm_en", "mem_idx=$com_idx"); 	
						
						if ($loadPage == "30_20"|| $loadPage == "30_22" || $loadPage == "30_23") { //선적 완료 페이지에서는 최근 history가 뭔지(수령/거절/수량부족 이면 안보여야함.)에 따라 checkbox가 보이고 안보이고 차이가 있다.)
							$chkbox_visible = "Y";
					...
					?>
						...
						<?if($loadPage!="30_20" || ($loadPage=="30_20" &&$chkbox_visible=="Y")){?>
					...
					<td colspan="12">
						<table class="detail-table">
							<tbody>
								<?if ($loadPage != "05_04_1" && $loadPage != "08_02" && $loadPage !="10_02" && $loadPage!="10_04" && $loadPage != "13_04"&& $loadPage != "03_02"){?>
								<tr class="noinput">
									<th scope="row" style="width:70px">부품상태  : </th>
									<td><span lang="en"><?=GF_Common_GetSingleList("PARTCOND",$part_condition)?></span></td>
									<th scope="row" style="width:70px">포장상태 : </th>
								...
								<?}?>
								<?if ($loadPage != "03_02" && $loadPage!="05_04_1"){?>
								<tr class="noinput">
									<td colspan="4" lang="en"><strong class="c-black">Memo : </strong><?=$memo?> </td>
								</tr>
								<?}?>
...
<!-- layer-step -->
	...
	<div class="layer-step">
		...
		<?		
			...
			if ($amend_yn =="N" && $det_cnt >="1"|| $amend_yn == "Y" && $startmodi== true){
	...
	function layerFile($loadPage,$reg_mem_idx , $reg_rel_idx , $odr_idx, $odr_history_idx){
		global  $pay;
		?>
	<!-- layer-file -->
		...
		<table>
			<tbody>
				<tr><?
				   ...
				   <?	    echo layerInvListData($loadPage ,$odr_idx,$odr_det_idx,$odr_history_idx);
						    break;
							case "13_04": // 취소 동의. 구매자에게 판매자 계약금 지급동의?>
	...
	<?}
	//---------------------------------------------------------------------------------------------------------------------------------------------------------
	function layerOrdListData($loadPage ,$odr_idx , $odr_det_idx=""){
		?>
		<!-- layer-data -->
	<input type="hidden" name="odr_idx" id="odr_idx_<?=$loadPage?>" value="<?=$odr_idx?>">
	...
	//-------------------------------------------------------------------------------------------------------------------------------------------
	function layerInvListData($loadPage ,$odr_idx, $odr_det_idx="" , $odr_history_idx=""){
		global $session_mem_idx;	
	?>
	<!-- layer-data -->
		...
		<table class="stock-list-table" id="list_<?=$loadPage?>">
			<thead>
				<tr>
					...
					<?if ($loadPage!="18R_06"){?><th scope="col" style="width:80px">Nation</th><?}?>
					<th scope="col" class="t-lt">Part No.</th>
					...
					<?}?>
					<?if ($loadPage!="31_06" && $loadPage!="18R_06"&& $loadPage!="04_01" && $loadPage!="03_02"){?><th scope="col">Company</th><?}?>
				</tr>
			</thead>
			<?	for ($i = 1; $i<=7; $i++){
	...
	function shipping_info($odr_idx, $loadPage=""){
		...
		<?}elseif($loadPage!="09_03"){?>
			<tr class="bg-none">
                ...
                <td></td>				
				<td colspan="12">		
				 <div class="txt_option" style="display:none;"><img src="/kor/images/txt_option.gif" alt="선적Option을 선택하여 발주서를 각각 발행할 수 있습니다.발주서 기준 최종 납기 제품과 일괄 배송됩니다." /></div>
						...
						<div class="company-info-wrap" style="display:<?if (!$delivery_addr_idx){echo"none";}?>">
							...
							<tr>
								...
								<td>
									...
									<div class="select type4" lang="en" style="width:92px">
										<label class="c-blue"><?
										if($assign_idx){
							...
							<tr >
								<td  colspan="4"><label class="ipt-chk chk2"><input <?if ($ship_info=="6") {echo "disabled";}?> type="checkbox" id="insur_yn" name="insur_yn" <?if ($insur_yn=="o"){echo "checked class='checked'";}?>><span></span> 운송보험</label> <span class="c-red"> <?if ($insur_yn=="o"){echo ": Yes";}else{echo ": No";}?></span></td>
							</tr>
							<tr >
								<td  colspan="4"><label class="ipt-chk chk2 com-chck"><input type="checkbox" <?if ($ship_info=="6") {echo "disabled";}?> name="delivery_chg" id="delivery_chg"  <?if ($delivery_addr_idx){echo "checked class='checked'";}?>><span></span> 배송지 변경</label></td>
							</tr>
						</tbody>
					</table>
