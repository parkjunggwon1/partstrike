...
$pay = "";
//***************************************************** GET_ODR_DET_LIST *****************************************************************
function GET_ODR_DET_LIST($loadPage, $part_type, $searchand, $det_cnt = 0, $first_yn = ""){   //part_type별 odr list 출력 (05_04 ,30_06, 30_08,30_15등등 layer에서 사용)
	...
	if ($cnt > 0){	
		while($row = mysql_fetch_array($result)){
			...
			?>
				...
				<!------------------------------------- 아이템 종류 ------------------------------------------------------------------------>
				<tr>
					<td colspan="<?=$colspan?>" class="title-box <?=($part_type=="1" || $first_yn=="1")? "first":"";?>">
			...
			<tr id="tr_<?=$odr_det_idx?>"><!------------ 내용 첫 번째 tr -------------->
				<?if ($loadPage== "30_06"|| $loadPage== "09_03" || $loadPage== "30_22"|| $loadPage== "31_04" || $loadPage== "01_29" ){ //판매자 페이지에서 보여지는 내용(30_06 layer)?>
					...
					<?if ($loadPage == "30_22"){
						...
						echo $quantity==0?"":number_format($quantity);
					}
					?></td>
					...
					<?}else{?>
					<td class="c-blue">$<?=number_format($odr_quantity*$price,2)?></td>
					<?}?>
				<?}elseif ($loadPage== "31_05"){?>
					...
					<td><input type="text" class="i-txt4" id="package" value="<?=$package?>" maxlength="10"style="width:64px" ></td>
					<td><input type="text" class="i-txt4" id="dc" value="<?=$dc?>" style="width:38px" maxlength="4" ></td>
					<td><?=$rhtype?></td>
					<td class="t-rt"><?=$quantity==0?"":number_format($quantity)?></td>
					<td class="t-rt">$<?=number_format($price,2)?></td>
					<td class="c-blue"><?=number_format($odr_quantity)?></td>
					<td><input type="text" id = "supply_quantity" name="supply_quantity" class="i-txt4 c-red2 onlynum numfmt t-rt" maxlength="10" value="<?=$odr_quantity==0?"":number_format($odr_quantity)?>" <?if ($part_type!=2){?>readonly<?}?>  style="width:58px"></td>
					...
					<td>
						<label class="ipt-chk chk2">
							<input type="<?=($det_cnt>1)? "checkbox":"hidden";?>" name="odr_det_idx[]" odr_status="<?=$odr_status;?>" quantity="<?=$quantity;?>" amend_yn="<?=$amend_yn?>" class="<?=($part_type=="2" && $period*1> 2 && QRY_CNT("odr_history", "and  odr_idx = $odr_idx and status = 19 ")<=0) ? "endure":"stock"?>" value="<?=$odr_det_idx?>" part_type="<?=$part_type?>"><span></span>
						</label>
					</td>
					<td><?=$i?></td>
					...
					<td>
						<input type="text" class="i-txt2 c-blue  onlynum numfmt t-rt" maxlength="10" name="odr_quantity[]" odr_det_idx="<?=$odr_det_idx?>" supply_quantity="<?=$supply_quantity;?>" value="<?=$odr_quantity==0?"":number_format($supply_quantity)?>" style="width:56px;"><!--기본 값을 공급수량과 동일-->
					</td>
					<td class="c-red"><?=$supply_quantity==0?"":number_format($supply_quantity)?></td>
						...
						<td colspan="13">
							...
							<table class="detail-table">
								<tbody>
									...
									<?}?>
									<?if ($loadPage != "03_02" && $loadPage!="05_04_1" && $loadPage!="13_04s"){?>
									<tr class="noinput">
										<td colspan="4" lang="en"><strong class="c-black">Memo : </strong><?=$memo?> </td>
									</tr>
									<?}?>
								</tbody>
							</table>
					...
					<td class="t-rt">
						...
						<input type="text" class="i-txt0 c-blue  onlynum numfmt t-rt" maxlength="10" name="odr_quantity[]" odr_det_idx="<?=$odr_det_idx?>" value="<?=$odr_quantity==0?"":number_format($odr_quantity)?>" style="width:58px;" readonly>
						<?}else{?>
						<input type="text" class="i-txt2 c-blue  onlynum numfmt t-rt" maxlength="10" name="odr_quantity[]" odr_det_idx="<?=$odr_det_idx?>" value="<?=$odr_quantity==0?"":number_format($odr_quantity)?>" style="width:58px;">
						<?}?>
					</td>
				...
				<?}elseif  ($loadPage== "30_08"){ //-------------- 송장(30_08) : 판매자 페이지에서 보여지는 송장 내용?>
					<?if($det_cnt>1){?>
						<td>
							<label class="ipt-chk chk2">
								<input type="checkbox" name="odr_det_idx[]" class="stock" odr_quantity="<?=$odr_quantity;?>" value="<?=$odr_det_idx?>"><span></span>
							</label>
						</td>
					<?}else{?>
					...
					<?}else{?>
					<td class="t-lt"><input type="text" class="i-txt4" name="part_no[]" value="<?=$part_no?>" maxlength="24" style="width:<?=($det_cnt>1)? "204":"233";?>px; ime-mode:disabled" ></td>
					<td class="t-lt"><input type="text" class="i-txt4" name="manufacturer[]" value="<?=$manufacturer?>" maxlength="20" style="width:<?=($det_cnt>1)? "166":"194";?>px; ime-mode:disabled" ></td>
					<td><input type="text" class="i-txt4" name="package[]" value="<?=$package?>" maxlength="10" style="width:75px; ime-mode:disabled" ></td>
					<td><?=$dc?></td>
					<td><?=$rhtype?></td>
					<td class="t-rt"><?=$quantity==0?"":number_format($quantity)?></td>
					<?}?>
					...
					<!--공급수량-->
					<td class="t-rt"><input type="text" name="supply_quantity[]" class="i-txt4 c-red2 onlynum numfmt t-rt" value="<?=$odr_quantity==0?"":number_format($odr_quantity)?>" maxlength="10" style="width:64px"></td>
								...
								<tbody>
									<tr <?=$part_type=="2"?"disabled":""?>>
										<th scope="row" style="width:195px">
											부품상태&nbsp;:&nbsp;&nbsp;<div class="select type4" lang="en" style="width:110px">
									...
									<tr>
										<td colspan="2" lang="en"><strong class="c-black">Memo : </strong> <input type="text" class="i-txt5" name="memo[]" value="" style="width:388px"></td>
									</tr>
								</tbody>
							</table>
						</td>
				<?}elseif  ($loadPage== "po_cancel"){ //-------------- 판매자 송장 품목 취소(삭제)?>
					<td><?=$i?><input type="hidden" name="odr_det_idx[]" value="<?=$odr_det_idx;?>"></td>
					<?if ($part_type=="7"){ //턴키--?>
						<td colspan="6"><input type="text" name="part_no[]" class="i-txt4" value="<?=$part_no?>" maxlength="30" style="width:584px; ime-mode:disabled" ></td>
					<?}else{?>
						...
						<td colspan="11">
							<table class="detail-table">
								<tbody>
									...
									<tr>
										<td lang="en" class="t-lt"><strong lang="ko" class="c-black">사유&nbsp;:&nbsp;</strong>&nbsp;<input type="text" class="i-txt2 c-blue w100" name="reason[]" value="" style="width:294px"></td>
									</tr>
									<tr>
										<td lang="ko" class="c-red2">취소 시 ‘발주 취소’ 항목의 숫자가 증가할 것입니다.</td>
									</tr>
								</tbody>
							</table>
						</td>
					...
					<?}elseif ($loadPage == "30_16" ||$loadPage=="18R_21"){ //[판매자] 선적(30_16)/교환선적시 보여지는 내용?>
						<td>
							<label class="ipt-chk chk2">
								<input type="<?=($det_cnt>1)? "checkbox":"hidden";?>" name="odr_det_idx[]" class="stock" odr_quantity="<?=$odr_quantity;?>" value="<?=$odr_det_idx?>"><span></span>
							</label>
						</td>
						<td><?=$i?></td>
						<?if ($part_type=="7"){?>
						...
						<?}else{?>
							...
							<td class="t-lt"><?=$manufacturer?></td>
							<td><?=$package?></td>
							<td><?if ($loadPage == "18R_21"){ //교환일 경우에는 제작년도도 input으로 다시 표시?><input type="text" name="fault_dc" maxlength="4" class="i-txt4 c-red2 onlynum" value="<?=$dc?>" style="width:38px"><?}else{?><?=$dc?><?}?></td>	<td><?=$rhtype?></td>
							<td class="t-rt"><?=$quantity==0?"":number_format($quantity)?></td>				
						<?}?>
						<?if ($loadPage == "18R_21"){ //교환일 경우에는 교환 수량만 표시
												global $fault_select;
												
												$fault_quantity = $fault_select =="3" ? $fault_quantity: $odr_quantity;
												?>
						...
						<?}else{?>
							<td class="t-rt">$<?=number_format($price,2)?></td>
							<td class="c-blue"><?=$odr_quantity==0?"":number_format($odr_quantity)?></td>
							<td class="c-red"><?=$supply_quantity==0?"":number_format($supply_quantity)?></td>
						<?}?>
						<?=($period)?"<td class='c-red'>".$period:(($part_type=="2"||$part_type=="5"||$part_type=="6")?"<td class='c-red'><span lang='ko'>확인</span>":"<td>Stock")?></td>
						<tr class="bg-none">
						...
						<td colspan="10">
							<table class="detail-table">
								...
								<tr class="yesinput"  style="display:none;">
									<td colspan="4" lang="en"><strong class="c-black">Memo : </strong> <input type="text" class="i-txt5" name="memo" value="<?=$memo?><?=$odr_det_idx?>_" style="width:294px"></td>
								</tr>
								<tr>
										<td colspan="4" class="img-cntrl-list">
											...
											<span class="img-wrap"><img  id="fileimg<?=$odr_det_idx?>_<?=$i?>" <?=get_noimg_photo($file_path, $file, "/kor/images/file_pt.gif")?> alt=""></span>
											<input name="file_o<?=$odr_det_idx?>_<?=$i?>" id="file_o<?=$odr_det_idx?>_<?=$i?>" type="hidden" value="<?=$file?>">
											<input name="file<?=$odr_det_idx?>_<?=$i?>" id="file<?=$odr_det_idx?>_<?=$i?>" type="file" style="display:none;">
					...
					?>
						...
						<?}else{?>
							...
							<td><?=$rhtype?></td>
						<?}?>
						<td class="t-rt"><?=$quantity==0?"":number_format($quantity); //수량?></td>
						<td class="t-rt">$<?=number_format($price,2)?></td>
						<td class="c-blue"><?=$odr_quantity==0?"":number_format($odr_quantity); //발주수량?></td>
						<td class="c-red"><?=number_format($supply_quantity) //공급수량?></td>
						<td><?=($period)? $period:"Stock";?></td>	
					<!-- 부가내용 시작-->
					</tr>
					<tr class="bg-none">
						<td></td>
					...
					?>
						...
						<?}else{?>
							...
							<td><?=$dc?></td>
							<td><?=$rhtype?></td>
							<?if($loadPage != "18R_19"){?><td class="t-rt"><?=$quantity==0?"":number_format($quantity)?></td><?}?>
						<?}?>
						<td class="t-rt">$<?=number_format($price,2)?></td>
						<td class="t-rt c-blue">
						<!-- 발주수량-->
							<?if($loadPage == "18_1_04"){?>
								<input type="text" name="fault_quantity" id="fault_quantity" class="i-txt4 c-red  onlynum numfmt t-rt" maxlength="10" Style="width:60px;" value="<?=$supply_quantity==0?"":number_format($supply_quantity)?>">
							<?}elseif($loadPage == "18R_19"){
						...
						<?if ($loadPage !="21_04" ){?>
								<?if($loadPage!="02_02"&& $loadPage !="05_04_1"){?><td class="t-rt c-red"><?=$supply_quantity==0?"":number_format($supply_quantity)?></td><?}?>
								<?=($period)?"<td class='c-red'>".(QRY_CNT("odr_history", "and  odr_idx = $odr_idx and status = 19 ")>0?"Stock":$period):(($part_type=="2"||$part_type=="5"||$part_type=="6")?"<td class='c-red'><span lang='ko'>확인</span>":"<td>Stock")?></td>
						<?}?>
						<?if ($loadPage!="31_06" && $loadPage!="02_02"){?>
						<?if ($loadPage !="18R_06" && $loadPage !="18R_19" && $loadPage != "03_02"&& $loadPage != "04_01"){?>
						<td class="c-blue"><?
							if($loadPage!="05_04_1" || ($loadPage=="05_04_1" && $sell_mem_idx != $_SESSION["MEM_IDX"])){
						?><a href="javascript:layer_company_det(<?=$com_idx?>);"><?=cut_len($company_nm,10,".");?></a><?
							}?></td><?
						}?>
					</tr>
					...
					<td colspan="12">
						<table class="detail-table">
							<tbody>
								<?if ($loadPage != "05_04_1" && $loadPage != "08_02" && $loadPage !="10_02" && $loadPage!="10_04" && $loadPage != "13_04" && $loadPage != "13_04s" && $loadPage != "13_02s" && $loadPage != "03_02"){?>
								<tr class="noinput">
									<th scope="row" style="width:200px">부품상태&nbsp;:&nbsp;&nbsp;<span lang="en" class="c-blue"><?=GF_Common_GetSingleList("PARTCOND",$part_condition)?></span></th>
									<th scope="row" style="width:200px">포장상태&nbsp;:&nbsp;&nbsp;<span lang="en" class="c-blue"><?=GF_Common_GetSingleList("PACKCOND1",$pack_condition1)?> <?=GF_Common_GetSingleList("PACKCOND2",$pack_condition2)?> </span></th>
								</tr>
								<?}?>
								<?if ($loadPage != "03_02" && $loadPage!="05_04_1" && $loadPage!="13_04s" && $loadPage!="13_02s"){?>
								<tr class="noinput">
									<td colspan="2" lang="en"><strong class="c-black">Memo : </strong><?=$memo?> </td>
								</tr>
								<?}?>
								<?if ($loadPage == "13_04s" || $loadPage == "13_02s"){?>
								<tr class="noinput">
									<td colspan="2" lang="en"><strong class="c-black">사유 : </strong><?=$det_reason;?> </td>
								</tr>
								<?}?>
								<?if ( $loadPage =="21_04" ){?>
								<tr>
						...
						?>
								...
								<?}elseif ( $loadPage == "19_06" || $loadPage =="19_08"  ||$loadPage =="18R_08" || $loadPage =="18R_19" || $loadPage =="18R_16"  ||$loadPage == "18R_06" || $loadPage == "18R_05" || $loadPage == "30_20" || $loadPage == "18_1_04"){?>
								<tr>
								...
								<tr class="yesinput"  style="display:none;">
									<td colspan="4" lang="en"><strong class="c-black">Memo : </strong> <input type="text" class="i-txt5" name="memo" value="<?=$memo?>" style="width:294px"></td>
								</tr>
...
if ($for_readonly != "P") {?>
	<ul class="total-price">
		...
		if ($part_type == 2 && $period*1 > 2 && $for_readonly=="" && $loadPage != "30_05"&& $loadPage != "12_07") { 
			if ($pay_cnt ==2){?>
				<li class="sub"><strong>Sub Total  :</strong><span>$<?=number_format($tot,2)?>	</li>
			<?
				$word ="Rest Amount(90%)";
				...
				$tot = $tot + $down;   //더하기. ( 왜냐하면 down 자체가 마이너스 값이니까)?>
				
				<li class="sub  c-red"><strong>Down Payment :</strong><span>$<?=number_format($down,2)?>	</li>					
			<?}else{
				$tot = $tot / 10;
				$charge_type = "2";
				?>
				<li class="sub"><strong>Down Payment :</strong><span>$<?=number_format($tot,2)?>	</li>							
			<?}
		}else{
		?>
			<li class="sub"><strong>Sub Total :</strong><span id="sub_total">$<?=number_format($tot,2)?></span></li>		
		<?}?>
		...
		<li class="total"><strong>Total :</strong><span id="g_total">$<?=number_format($tot,2)?></span></li>
...
<!-- layer-step(거래단계 표기) -->
	...
	<?if ($loadPage!="31_04"){ //납기확인 전까지는 history 필요없댐.?>
	<div class="layer-step">
		...
		<?		
			...
			if ($amend_yn =="N" && $det_cnt >="1"|| $amend_yn == "Y"){ //2016-04-19 : 수정발주서 여부 상관없이 전체 히스토리 표시
				...
				if ($session_mem_idx != $reg_mem_idx){$cls = "class='c1$red'";}
				if ($status== "9" || $status== "10" || $status== "22" || $status =="11"|| $status =="24"){
					$cls = "class='c2$red'";
				...
				?>
					<li <?=$cls?>>
						...
						<span class="etc"><span lang="en"><?if ($etc1){echo openSheet($status, $etc1,$odr_idx);}?></span></span>
						<span class="etc"><span lang="en"><?if ($etc2){?><?=$etc2?><?}?></span></span>
					</li>
			<?}
		}  //end of while?>
		</ol>
			...
			<tbody>
				<tr><?
					...
					<?	  
						  ...
						  case "09_03":?>
					<td class="company"><img src="/kor/images/nation_title_<?=$buy_com_nation?>.png" alt="<?=GF_Common_GetSingleList("NA",$buy_com_nation)?>"> <span class="name"><?=$buy_com_name?></span></td>
					<td class="c-red2 w100 t-ct">수정발주서가 도착했습니다.</td>				
					</tr></tbody></table></div>
					<?	  
						   echo layerOrdListData($loadPage ,$odr_idx);
						   break;
						case "30_10":?>
					<td class="c-red2 t-ct">송장이 도착했습니다.</td>
					</tr></tbody></table></div>	
					<?
						   echo layerInvListData($loadPage ,$odr_idx);
						   break;	
						case "30_15":  //-----------------------------------------30_15 What's New : 결제 완료 ------------------------------------------------------------------
					?>
					<td class="company"><img src="/kor/images/nation_title_<?=$buy_com_nation?>.png" alt="<?=GF_Common_GetSingleList("NA",$buy_com_nation)?>"> <span class="name"><?=$buy_com_name?></span></td>
					<td class="c-red2 w100 t-ct">
					<?if ($_SESSION["MEM_IDX"] == $odr_his[buy_mem_idx] && $with_deposit=="Y"){
						echo "보증금, 총 금액 (<strong class='c-blue'><span lang='en'>US$"; 
						$deposit ="1000";
						echo number_format(floatval(trim(str_replace("입금","",str_replace("$","",$pay)))) + $deposit,2);
					}else{$deposit=0;
						echo "총 금액 (<strong class='c-blue'><span lang='en'>US$".$pay; 
					}?></span></strong>) 결제가 완료되었습니다.</td>
					</tr></tbody></table></div>	
					<?
							echo layerInvListData($loadPage ,$odr_idx,$odr_det_idx,$odr_history_idx);  //JSJ
							//echo layerOrdListData($loadPage ,$odr_idx,$odr_det_idx,$odr_history_idx);  //2016-04-20
						   break;	
						case "19_1_05":  //----------------------------------------------------------------------------------------------------------
					?>
					<td class="company"><img src="/kor/images/nation_title_<?=$buy_com_nation?>.png" alt="<?=GF_Common_GetSingleList("NA",$buy_com_nation)?>"> <span class="name"><?=$buy_com_name?></span></td>
					<td class="c-red2 w100 t-ct">총 금액 (<strong class="c-blue"><span lang="en">US$ <?=$pay?></span></strong>) 결제가 완료되었습니다.</td>
					</tr></tbody></table></div>	
					<?
						   echo layerInvListData($loadPage ,$odr_idx);
						   break;	
					case "30_20": //----------------------------------- 선적 완료(구매자) --------------------------------------------------------------------------
					$delivery_no	 = get_any("ship","delivery_no", "odr_idx = $odr_idx");
					$ship_info	 = get_any("ship","ship_info", "odr_idx = $odr_idx");
					?>
					<td class="c-red2">운송회사 : <img src="/kor/images/icon_<?=strtolower(GF_Common_GetSingleList("DLVR",$ship_info))?>.gif" height="15" alt=""> &nbsp;&nbsp;&nbsp;운송장번호 : <span lang="en" class="c-blue"><?=$delivery_no?></span></td>
					...
					<?
					   ...
					   case "30_22":?>
					<td class="company"><img src="/kor/images/nation_title_<?=$buy_com_nation?>.png" alt="<?=GF_Common_GetSingleList("NA",$buy_com_nation)?>"> <span class="name"><?=$buy_com_name?></span></td>
					<td class="c-red2 w100 t-ct">구매자가 제품을 문제없이 수령하였습니다.</td>
					</tr></tbody></table></div>	
					<?
						   echo layerOrdListData($loadPage ,$odr_idx);
						   break;
						...
						?>
							<td colspan="2">
								<table style="border:0; padding:5px;">
									<tr>
										<td Style="width:600px;" class="c-red2 t-lt">운송회사 : <img src="/kor/images/icon_<?=strtolower(GF_Common_GetSingleList("DLVR",$ship[ship_info]))?>.gif" alt=""> &nbsp;&nbsp;&nbsp;<span lang="en">Account No.</span> <span lang="en" class="c-blue"><?=$ship[ship_account_no]?></span></td>
										<td>
											<!-- 운송 기타정보-->
											<table>
				   ...
				   <?	    echo layerInvListData($loadPage ,$odr_idx,$odr_det_idx,$odr_history_idx);
					...
					?>
							<td class="t-ct w100 c-red">&nbsp;</td><!-- 2016-04-13 : 일단 메세지 없이 -->
							</tr></tbody></table></div>	
							<?echo layerInvListData($loadPage ,$odr_idx,$odr_det_idx,$odr_history_idx);
						    break;							
						   case "10_04":   //-------------------------------------------------------------납기 연장 확인 ------------------------------------------------
		...
		<table class="stock-list-table">
			...
			<?
				if ($odr_det_idx && $odr_det_idx !="0"){
					...
					$q_cnt = QRY_CNT("odr_det a left outer join part b on  a.part_idx = b.part_idx "," and odr_idx=$odr_idx $searchand and b.part_type =$i ");
					if($q_cnt>0) $first_yn++;
					echo GET_ODR_DET_LIST($loadPage,$i," and odr_idx=$odr_idx $searchand","",$first_yn);
					if($q_cnt>0) $first_yn++;
			}
			
			if ($loadPage == "30_06" ||$loadPage=="09_03"){//----------------- 선적정보 -----------------------------
						...
						<tbody>
							<tr>
								<th scope="row" style="width:180px">
									선적정보 : <span class="c-grey2">운송회사</span> <img src="/kor/images/icon_<?=strtolower(GF_Common_GetSingleList("DLVR",$row_ship["ship_info"]))?>.gif" alt="" height="15">
								</td>
								<th scope="row" style="width:200px">
									<span lang="en">Account No : </span>
									<span lang="en"><?=$row_ship["ship_account_no"]?></span>
								</th>
							</tr>
							<tr>
								<td colspan="2" lang="en"><strong class="c-black">Memo : </strong><?=$row_ship["memo"]?></td>
							</tr>
							<tr>
								<th colspan="2" scope="row">
									운송보험 : <span lang="en"><?=$row_ship["insur_yn"]=="o"?"Yes":"No"?></span>
								</th>
							</tr>
						</tbody>
					</table>
	...
	<div class="btn-area t-rt">
		<a href="#" class="btn-view-<?if($loadPage=="09_03"){?>amend-<?}?>sheet-forread" odr_idx="<?=$odr_idx?>"><img src="/kor/images/btn_order_confirm.gif" alt="발주서 확인"></a>
		<?if($loadPage== "09_03"){?>
		<a href="#" class="btn-pop-0601"><img src="/kor/images/btn_cancel.gif" alt="취소"></a>
		<?}?>
	</div>
	<!-- //layer-btn -->
	<?}elseif ($loadPage =="30_22"){?>
		<div class="btn-area t-rt">
			<a href="#" class="btn_3022"  odr_idx="<?=$odr_idx?>"><img src="/kor/images/btn_complete.gif" alt="완료"></a>
		</div>
	...
	<div class="layer-data" id="data_<?=$loadPage?>">
		<table class="stock-list-table" id="list_<?=$loadPage?>">
			<thead>
				<tr>
					<?if ($loadPage =="04_01" || $loadPage =="30_20"){?><th scope="col" style="width:50px">Option</th><?}?>
					<th scope="col" class="t-no">No.</th>
					<?if ($loadPage!="18R_06" && $loadPage!="30_15"){?><th scope="col" style="width:80px">Nation</th><?}?>
					<th scope="col" class="t-partno">Part No.</th>
					<th scope="col" class="t-Manufacturer">Manufacturer</th>
					<th scope="col" class="t-Package">Package</th>
					<th scope="col" class="t-dc">D/C</th>
					<th scope="col" class="t-rohs">RoHS</th>
					<?if($loadPage!="18R_19"){?><th scope="col" class="t-oty">O'ty</th><?}?>
					<th scope="col" class="t-unitprice">Unit Price</th>
					<?if ($loadPage=="21_04"){?>
						<th scope="col" lang="en">Amount</th>
					<?}else{?>
						
						<th scope="col" lang="ko" class="t-orderoty"><?=($loadPage == "18_1_04" || $loadPage == "18R_19")? "반품수량":"발주수량";?></th>
						<th scope="col" lang="ko" class="t-supplyoty">공급수량</th>
						<th scope="col" lang="ko" class="t-period">납기</th>
					<?}?>
					<?if ($loadPage!="31_06" && $loadPage!="18R_06"&& $loadPage!="04_01" && $loadPage!="03_02" && $loadPage!="30_15" && $loadPage!="18R_19"){?>
			...
			<?	for ($i = 1; $i<=7; $i++){
				...
				echo GET_ODR_DET_LIST($loadPage,$i," and odr_idx=$odr_idx ".($odr_det_idx!="" && $odr_det_idx !="0"?"and odr_det_idx=$odr_det_idx":"$sql"));
			}
			?>             
		</table>        
	</div>

	<?if ($loadPage =="18R_08" || $loadPage =="18R_06" || $loadPage =="18R_16" || $loadPage =="18R_19"){  //--------------- 거절(18R_08) 메시지 내역(주고, 받고) -----------------------------------------------------------------------------------------------
	?>
	<div class="layer-data">
		<table class="stock-list-table">
			<tr class="bg-none">
			...
			<?
			//거절 메세지 순서대로 가져오기...
			$result = QRY_RCD_MSG_LIST($odr_det_idx, 9);
			?>
			<tbody>
				<?
				$i=0;
	...
	<?if ($loadPage =="01_37"){?>
	<hr class="dashline2">
	<p class="c-red2 t-ct">추가 발주 수량 : <input type="text" id="req_addcapa" maxlength="10" class="i-txt2 t-ct c-blue onlynum numfmt t-rt" style="width:100px" value="0" <?if (get_any("odr_det","add_capa_quantity","odr_idx=$odr_idx")=="0"){?>readonly<?}?> onfocus="if(this.value=='0'){this.value=''}" onblur="if(this.value==''){this.value='0'}" > EA</p>
	<?}?>
	<?if ($loadPage =="30_20"){  //------------------------------ 30_20 버튼 (What's New : 선적완료) ----------------------------------------------------------------------------------------------
	?>
	<hr class="dashline2">	
	...
	?>
		...
		-->
	<?}elseif ($loadPage=="03_02" || $loadPage=="19_1_06"){?>
		<a href="#" class="complete<?if ($loadPage=="19_1_06"){?>Only<?}?>" odr_history_idx="<?=$odr_history_idx?>"><img src="/kor/images/btn_complete.gif" alt="완료"></a>		
	...
	?><button type="button" class="<?=$loadPage=="10_04"?"sell-mn05":"btn-close"?>"><img src="/kor/images/btn_ok.gif" alt="확인"></button>
...
<?			}		
	...
	<?
		...
		if($fault_select=="2"){ //-- 반품 -------------
			...
			$invoice_no = $odr_det[non_com_invoice];
			...
			?>
			<button class="btn-pop-2001" type="button" his_type="<?=$his_ty?>" history_idx="<?=$fty_history_idx?>" invoice_no="<?=$invoice_no?>"><img alt="PARTStrike" src="/kor/images/btn_parts.gif"></button>
			<?}?>
			<?
			?>
	...
	<?}elseif($loadPage == "19_08"){?>
		<a href="#" class="btn-dialog-1908"><img src="/kor/images/btn_reply.gif" alt="답변서"></a>
...
<?	
	...
	function shipping_info($odr_idx, $loadPage=""){
		...
		if ($loadPage=="09_03" && $delivery_addr_idx){?>
			<hr class="dashline2">
					...
					<table class="detail-table">
						...
						<div class="company-info-wrap" style="display:<?if (!$delivery_addr_idx){echo"none";}?>">
							...
							<tr>
								...
								<th scope="row"><span lang="en" style="display:<?if ($ship_info == "5" || $ship_info == "6"){?>none<?}?>;">Account No.</span></th>
                            ...
                            <tr>
                                <th></th>                                
                                <td colspan="3"><span style="color: #666666; font-size:11px; padding-left:50px;">해당 운송업체로만 선적이 가능한 회사입니다.<input type="hidden" name="ship_info" value="<?=$assign_idx?$assign_idx:$ship_info?>"></span></td>
                            </tr>
							<?}?>
							<tr>
								<td colspan="4" lang="en"><strong class="c-black">Memo : </strong> <input type="text" class="i-txt<?=$ship_info=="6" || $ship_info=="5"?"2":"5"?>" id ="memo" name="memo" value="<?=$memo?>" style="width:367px"></td>
							</tr>
							<tr>
							...
							<tr >
								<td  colspan="4"><label class="ipt-chk chk2 com-chck" Style="margin-left:38px;"><input type="checkbox" <?if ($ship_info=="6") {echo "disabled";}?> name="delivery_chg" id="delivery_chg"  <?if ($delivery_addr_idx){echo "checked class='checked'";}?>><span></span> 배송지 변경</label></td>
							</tr>
						</tbody>
					</table>
	...
	function GET_ODR_DELIVERY_ADDR($delivery_addr_idx){
		...
		?>
		<table class="company-info-tb" style="width:745px" align="center">
				<tbody>
					<tr>
						<th scope="row">국가</th>
	...
	<?}
	//배송지 변경
	function GET_CHG_ODR_DELIVERY_ADDR($delivery_addr_idx){
		if ($delivery_addr_idx){
			$result = QRY_DELIVERY_ADDR_VIEW($delivery_addr_idx);
			$row = mysql_fetch_array($result);
			if ($row){
	...
	<script type="text/javascript">
		<!--
			...
			$(".company-info-wrap select").change(function(){
				CalculChk();
			});
				...
				if (MustChk() == true)
					{
						...
						$("#f .info-btn-area button:eq(0)").hide();
						$("#f .info-btn-area span:eq(0)").show();
					}
			...
			{		
				...
				if(f.com_name.value==""){ return "com_name";}
			...
			<?if ($saved_cnt>=15 && $delivery_addr_idx<=0){?>
				$(document).ready(function(){
					$(".company-info-wrap input").attr("disabled",true);
					$(".company-info-wrap select").attr("disabled",true);
				});
			<?}?>
	...
	<table class="company-info-tb" style="width:745px">
		<tbody>
			<tr>
				...
				<td colspan="2">
					<div class="select type6 bd2" lang="en" style="width: 215px;">
						<label class="c-blue"><?=($nation)?GF_Common_GetSingleList("NA",$nation):""?></label>
						<?echo GF_Common_SetComboList("nation", "NA", "", 1, "True",  "", $nation  , "onchange='chgnation(this);' lang='en'");?>
					</div>
				</td>
			</tr>
			<tr>
				<th scope="row"><strong class="c-red">*</strong> 회사명</th>
				<td colspan="2"><input class="i-txt5 c-blue" maxlength="30" lang="en" type="text" name="com_name" style="width:215px" value="<?=$com_name?>"></td>
			</tr>
			<tr>
				<th scope="row"><strong class="c-red">*</strong> 담당자</th>
				<td colspan="2"><input class="i-txt5 c-blue" type="text" maxlength="30" lang="en" name="manager" style="width:215px" value="<?=$manager?>"></td>
			</tr>
			<tr>
				<th scope="row"><strong class="c-red"></strong> 직책</th>
				<td colspan="2"><input class="i-txt5 c-blue" type="text" maxlength="30" lang="en" name="pos_nm" style="width:215px" value="<?=$pos_nm?>"></td>
			</tr>
			<tr>
				<th scope="row"><strong class="c-red"></strong> 부서</th>
				<td colspan="2"><input class="i-txt5 c-blue" type="text" name="depart_nm" lang="en" style="width:215px" value="<?=$depart_nm?>"></td>
			</tr>
			<tr>
				...
				<td colspan="2">
					<div class="select type6 bd2" lang="en" style="width: 215px;">
						...
						<?echo GF_Common_SetComboList("com_type", "MEM", "", 1, "True",  "", $com_type  , "lang='en'");?>
					</div>
				</td>
			</tr>
			<tr>
				<th scope="row"><strong class="c-red">*</strong> <span lang="en">Tel</span></th>
				<td colspan="2"><input class="i-txt5 c-blue" name="tel" type="text" maxlength="15" lang='en' style="width:215px" value="<?=$tel?>"></td>
			</tr>
			<tr>
				<th scope="row"><strong class="c-red"></strong> <span lang="en">Fax</span></th>
				<td colspan="2"><input class="i-txt5 c-blue" name="fax"  type="text" maxlength="15" lang='en' style="width:215px" value="<?=$fax?>"></td>
			</tr>
			<tr>
				<th scope="row"><strong class="c-red"></strong> 휴대전화</th>
				<td colspan="2"><input class="i-txt5 c-blue" name="hp" type="text" maxlength="15" lang='en' style="width:215px" value="<?=$hp?>"></td>
			</tr>
			<tr>
				<th scope="row"><strong class="c-red">*</strong> <span lang="en">E-mail</span></th>
				<td colspan="2"><input class="i-txt5 c-blue" name="email" type="text" maxlength="30" lang='en' style="width:215px"  value="<?=$email?>"></td>
			</tr>
			<tr>
				<th scope="row"><strong class="c-red"></strong> 홈페이지</th>
				<td colspan="2"><input class="i-txt5 c-blue" name="homepage" type="text" style="width:215px" lang='en' value="<?=$homepage?>"></td>
			</tr>
			<tr>
				<th scope="row"><strong class="c-red">*</strong> 우편번호</th>
				<td colspan="2"><input class="i-txt5 c-blue" name="zipcode" type="text" maxlength="10" style="width:215px"  lang='en' value="<?=$zipcode?>"><span class="c-red">&nbsp;&nbsp;
					...
					<button type="button" onclick="openCommLayer('layer4','layerZipNew','?frm_name=f&frm_zip1=zipcode&frm_addr1=addr&frm_addr2=addr_det');" class="roadname" style="display:<?if ($nation!="1"){echo "none";}?>;"><img src="/kor/images/btn_roadname.gif" border=0 align=absmiddle alt="도로명주소"></button>
					</span></td>
			</tr>
			<tr>
				<th scope="row"><strong class="c-red">*</strong> 도/시</th>
				<td colspan="2">
					<div class="select type6 bd2" lang='en' style="width: 215px;display:<?=($dosi!=""|| ($dosi=="" && $dositxt==""))?"":"none"?>;">
						<label class="c-blue"><?=($dosi)?GF_Common_GetSingleList_LANG("NA",$dosi,""):""?></label>
						<?=GF_Common_SetComboList("dosi", "NA", $nation, 2, "True",  "", $dosi , "onchange='chgdosi(this);' lang='en'");?>
					</div>

					<input type="text" onkeyup="chgdositxt(this,'');" class="i-txt5 c-blue" name="dositxt" id="dositxt" lang='en' value="<?=$dositxt?>" style="width:215px;display:<?=$dositxt!="" ?"":"none"?>;" >
				</td>
			</tr>
			<tr>
				<th scope="row"><strong class="c-red">*</strong> 시/군/구</th>
				<td colspan="2">
				    <input type="text" onkeyup="chgsigungu(this,'');" class="i-txt5 c-blue" maxlength="30" style="width:215px"  lang='en' name="sigungu" id="sigungu" value="<?=$sigungu?>">
				</td>
			</tr>
			<tr>
				<th scope="row"><strong class="c-red">*</strong> 주소</th>
				<td><input class="i-txt5 c-blue w100" name="addr" lang='en' value="<?=$addr?>" type="text"></td>
				<td style="width:120px" class="info-btn-area">
				<span><img src="/kor/images/btn_save_1.gif" alt="저장"></span><button style="display:none;" type="button" class="delivery_save"><img src="/kor/images/btn_save.gif" alt="저장" ></button> <?if($com_name){?><button type="button"  class="delivery_del"><img src="/kor/images/btn_delete3.gif" alt="삭제"></button><?}else{?><span><img src="/kor/images/btn_delete3_1.gif" alt="삭제"></span><?}?></td>
			</tr>
			<tr>
				<td colspan="3">
					<?echo GET_MY_ODR_DELIVERY_ADDR_LIST($delivery_addr_idx)?>
				</td>
			</tr>
		</tbody>
	</table>
	...
	function GET_MY_ODR_DELIVERY_ADDR_LIST ($idx){
