...
if ($cnt>0){
	...
	if ($page==1){
	  ...
	  $prev=$page-1;
	  $intpage .= "<li class='navi-prev'><a href='javascript:openCommLayer('$layerNum','$loadPage','$varNum&page=$prev');' class='link' num='$prev' alt='Prev' >Prev</a></li> ";	
	}
	...
	for ($np=$startpage; $np<=$endpage; $np++) {
		if ($np ==$page) { 
			...
			$intpage .= "<li><a href=\"javascript:openCommLayer('$layerNum','$loadPage','$varNum&page=$np');\" class='link' num='$np'>$np</a> ";
		}
	}
	$intpage .= "</li>";
	$next=$page+1;
	//[다음10개]페이지 보여주기
	 if ($intTemp<$totalpage){		
	    $intpage .= "<li class='navi-next'><a href='javascript:openCommLayer('$layerNum','$loadPage','$varNum&page=$next');' class='link' num ='$next' alt='Next' >Next</a></li> ";	    
     } else {
