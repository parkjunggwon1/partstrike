...
Function QRY_RCD_DET_LIST($recordcnt,$searchand,$page,$ord='odr_det_idx'){
	...
	switch($ord){
		case "odr_det_idx":
			...
			break;
		default:
			$s_ord=" order by (select max(reg_date) from odr_history where odr_history.odr_idx = a.odr_idx limit 1) desc,c.part_type,$ord";
			break;
	}

	$sql = "
			SELECT *,a.odr_status as order_status,c.quantity as part_stock,c.price as part_price,c.del_chk as del_chk FROM odr a 
