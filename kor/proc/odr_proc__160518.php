...
if ($typ=="write" || $typ=="odredit" ||$typ =="periodreq"){   //periodreq : 납기확인
	...
	**/
	...
	if ($odr_idx && $fromPage!="add"){  //Ship Update
	...
	*******************************************************************/
	...
	if ($typ =="periodreq") {  //납기 확인의 경우----------------------------------------------------------------------
		if($odr_idx_yn=="Y"){ //2016-04-05
			...
			$sql = "INSERT INTO odr_det (odr_idx, part_idx, part_type, odr_quantity, supply_quantity, fault_dc, rnd_yn, period)
					SELECT $new_odr_idx, part_idx, part_type, odr_quantity, supply_quantity, fault_dc, rnd_yn, period FROM odr_det WHERE odr_det_idx = $odr_det_idx ";
			$result=mysql_query($sql,$conn) or die ("SQL ERROR : ".mysql_error());
...
if($typ == "persave"){
	...
	$sql = "INSERT INTO odr_det (odr_idx, part_idx, part_type, odr_quantity, supply_quantity, fault_dc, rnd_yn, period, odr_status, rel_det_idx)
				SELECT $save_odr_idx, part_idx, part_type, odr_quantity, supply_quantity, fault_dc, rnd_yn, period, odr_status, rel_det_idx FROM odr_det WHERE odr_idx = $odr_idx AND odr_status NOT IN(16) ";
	$result=mysql_query($sql,$conn) or die ("SQL ERROR : ".mysql_error());
	//2-1. 상태 16인건 별도로 rel_det_idx 따주자.
	$sql = "INSERT INTO odr_det (odr_idx, part_idx, part_type, odr_quantity, supply_quantity, fault_dc, rnd_yn, period, odr_status, rel_det_idx)
				SELECT $save_odr_idx, part_idx, part_type, odr_quantity, supply_quantity, fault_dc, rnd_yn, period, odr_status, odr_det_idx FROM odr_det WHERE odr_idx = $odr_idx AND odr_status IN(16) ";
	$result=mysql_query($sql,$conn) or die ("SQL ERROR : ".mysql_error());
...
if($typ =="invconfirm2"){ //-------------------------------------- 판매자 : 송장 확정2:30_09 (Invoice) 2016-04-15 ---------------------------------------------------------------------------------------------------
	...
	update_val("odr","invoice_no",$inv_no , "odr_idx", $odr_idx);	
	**/
	$inv_no = get_any("odr", "invoice_no" , "odr_idx = $odr_idx"); //2016-04-18 : 'invreg' 에서 만든 송장번호 가져온다.
	$buy_mem_idx = get_any("odr", "mem_idx" , "odr_idx = $odr_idx");
	//3. histoy update(발주서를 확인 한 상태로 update)
	$odr_history_idx = get_any("odr_history" , "odr_history_idx", "odr_idx= $odr_idx and (status = 2 or status=3) and confirm_yn='N'");
	update_val("odr_history","confirm_yn","Y", "odr_history_idx", $odr_history_idx);
...
if ($typ =="odrconfirm2"){	//------------ 확정 발주서 (from:30_05) 2016-04-19 : log 기록(사본생성) --------------------------------------------------------------------	
		...
		$sql = "INSERT INTO odr (odr_no, imsi_odr_no, invoice_no, amend_no, amend_date, mem_idx, rel_idx, sell_mem_idx, sell_rel_idx, period, odr_status, turnkey_idx, accept_yn, cancel_accept_yn, ship_idx, fault_delivery_no, buyer_delivery_fee, faulty_delivery_fee, memo, save_yn, reg_date, status_edit_mem_idx, complete_yn, reg_ip,doc_no)
				SELECT '', '', '', '', '', mem_idx, rel_idx, sell_mem_idx, sell_rel_idx, period, '99', turnkey_idx, accept_yn, cancel_accept_yn, ship_idx, fault_delivery_no, buyer_delivery_fee, faulty_delivery_fee, memo, save_yn, reg_date, status_edit_mem_idx, complete_yn, reg_ip,'$odr_no'  FROM odr WHERE odr_idx = $odr_idx
				";
		$result=mysql_query($sql,$conn) or die ("SQL ERROR : ".mysql_error());
		$log_idx = mysql_insert_id();
		//odr_det : odr_idx=$log_idx
		$sql = "INSERT INTO odr_det (odr_idx, part_idx, part_type, odr_quantity, supply_quantity, add_capa_quantity, add_quantity, fault_quantity, fault_method, fault_part_condition, fault_pack_condition1, fault_pack_condition2, fault_dc, fault_memo, ship_idx, agreement_no, agreement_reg_date, rnd_yn, rework_invoice, rework_date, refund_invoice, refund_date, non_com_invoice, non_com_date, compensation_invoice, compensation_date, access_invoice, access_date, amend_invoice, amend_date, testS_invoice, testS_date, testB_invoice, testB_date, test_report_no, test_report_date, part_condition, pack_condition1, pack_condition2, memo, file1, file2, file3, period, amend_yn, odr_status, rel_det_idx)
				SELECT $log_idx, part_idx, part_type, odr_quantity, supply_quantity, add_capa_quantity, add_quantity, fault_quantity, fault_method, fault_part_condition, fault_pack_condition1, fault_pack_condition2, fault_dc, fault_memo, ship_idx, agreement_no, agreement_reg_date, rnd_yn, rework_invoice, rework_date, refund_invoice, refund_date, non_com_invoice, non_com_date, compensation_invoice, compensation_date, access_invoice, access_date, amend_invoice, amend_date, testS_invoice, testS_date, testB_invoice, testB_date, test_report_no, test_report_date, part_condition, pack_condition1, pack_condition2, memo, file1, file2, file3, period, amend_yn, odr_status, rel_det_idx
...
if($typ == "shipping2"){
	...
	//2. histoy update(반품선적 완료를 확인한 상태로 update)  수량 부족일 때에는 수량 부족을 확인 한 상태로 update
	$prev_status = ($fault_method == "3")? "10":"11";	//fault_method : 3 => 추가선적(수량부족) / status => 10:수량부족, 11:반품선적완료
	$prev_odr_history_idx = get_any("odr_history" , "odr_history_idx", "odr_idx= $odr_idx and status = '$prev_status' and confirm_yn = 'N'");
	update_val("odr_history","confirm_yn","Y", "odr_history_idx", $prev_odr_history_idx);
...
if ($typ =="succEnd2"){   //------------------ 2016-04-24 : det에 각각 히스토리 등록하던것을 odr 단위로 등록 ------------------------------------------------------
	$det_cnt = QRY_CNT("odr_det", "and odr_idx= $odr_idx");
	$session_mem_idx = $_SESSION["MEM_IDX"];
	$sell_mem_idx = get_any("odr", "sell_mem_idx" , "odr_idx = $odr_idx");
	//2. history 등록		
