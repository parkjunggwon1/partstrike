...
if ($typ=="write" || $typ=="odredit" ||$typ =="periodreq"){   //periodreq : 납기확인
	...
	**/
	...
	if ($odr_idx && $fromPage!="add"){  //추가가 아니면 발주??
	...
	*******************************************************************/
	...
	//		Page_Parent_Msg_Url("이미 발주 또는 납기 확인요청이 들어간 부품입니다.","/kor/");
			...
			if($fromLoadPage == "09_01" || $odr_status==16){ //09_01:수정발주서창, 16:납기 받은거창
				$amend_yn = "Y";
			}else{
				$amend_yn = "N";
			}
	...
	if ($typ =="periodreq") {  //납기 확인의 경우-------------[처리 및 Layer Opern]----------------------------------------------------------
		//-------------------------------- 신규:발주서 창이 납기 받은 제품 일 경우 ---------------------------------------------
		if($odr_status==16){
			//----------------------------------- 기존 납기 받은게 발주창에 있고, 납기품목 추가시 ----------------------------
			//1. insert odr
			$sql = "insert into odr set
					imsi_odr_no = 'IM-".date("ymdhms").RndomNum(4)."'
					,mem_idx = $session_mem_idx
...
if ($typ =="periodcfrm"){ //납기기한 확인
	...
	//2016-03-29 det_status Update
	update_val("odr_det" , "odr_status", 16, "odr_det_idx" , $odr_det_idx);  
