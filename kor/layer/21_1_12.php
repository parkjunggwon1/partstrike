...
<?  $summybank =  str_replace(",","",SumMyBank($session_mem_idx, $session_rel_idx));		
	...
	$rel_idx = get_any("odr", "rel_idx" , "odr_idx = $odr_idx");
	
?>
<form name="f" id="f">
	<!-- form1 -->
	<input type="hidden" name="typ" id="typ" value="pay">
	<input type="hidden" name="sell_mem_idx" id="mem_idx" value="<?=$session_mem_idx?>">
	<input type="hidden" name="sell_rel_idx" id="rel_idx" value="<?=$session_rel_idx?>">	
	<input type="hidden" name="tot_amt" id="tot_amt" value="<?=$tot_amt?>">		
	<input type="hidden" name="mem_idx" id="sell_mem_idx" value="<?=$mem_idx?>">		
	<input type="hidden" name="rel_idx" id="sell_mem_idx" value="<?=$rel_idx?>">		
