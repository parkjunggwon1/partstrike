...
//판매자 책임 부대비용 송장 sheet
 ...
 if($row_odr_det["access_invoice"]==""){ 
	  $sql = "update odr_det set access_invoice= '".get_auto_no("ACI", "odr_det" , "access_invoice")."', access_date = now() where odr_det_idx=".$odr_det_idx;
	  $result = mysql_query($sql,$conn) or die ("SQL Error : ". mysql_error());
	   $result_odr_det =QRY_ODR_DET_LIST(0,"and odr_det_idx = ".$odr_det_idx,0); 
		  $row_odr_det = mysql_fetch_array($result_odr_det);
  }

  $result_buyer = QRY_ODR_MEMBER_VIEW($odr_idx,"idx",($row_odr["rel_idx"]==0?$row_odr["mem_idx"]:$row_odr["rel_idx"]),$row_odr_det["access_invoice"]);
  $row_buyer = mysql_fetch_array($result_buyer);

  $result_seller = QRY_ODR_MEMBER_VIEW($odr_idx,"idx",($row_odr["sell_rel_idx"]==0?$row_odr["sell_mem_idx"]:$row_odr["sell_rel_idx"]),$row_odr_det["access_invoice"]);
  $row_seller = mysql_fetch_array($result_seller);

	$result_parts = QRY_ODR_MEMBER_VIEW($odr_idx,"idx",get_any("member", "min(mem_idx)", "mem_type = 0"),$row_odr_det["access_invoice"]); //파츠 회사 정보
...
<div class="sheet-wrap">
	...
	<div class="order-info"
		<ul>
			<li class="b1"><strong>Accessorial Charge Invoice No.</strong><span><?=$row_odr_det["access_invoice"]?></span></li>
			<li class="b2"><strong>Date</strong><span><?=$row_odr_det["access_date"]?></span></li>
	...
	<div class="order-table">
		...
		<?echo GET_ODR_DET_LIST_V2(" and odr_idx=$odr_idx and odr_det_idx =$odr_det_idx" ,"18_2_09"); ?>			
	</div>
	<!-- //order-table -->
	...
	<div class="btn-area t-rt">
		...
		<div class="f-rt">			
			<button type="button" class="btn-pop-3012" odr_idx="<?=$odr_idx?>" odr_det_idx="<?=$odr_det_idx?>" tot_amt="<?=$tot_amt?>" fromLoadPage="18_2_09"><img src="/kor/images/btn_payment.gif" alt="결제"></button>
		</div>
	</div>
</div>
