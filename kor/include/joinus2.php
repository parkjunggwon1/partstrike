	...
	function fileChange(){
		 $("input[type=file]").change(function(){	
			...
			if(no_val > 3){
				...
				$(".plus_chk"+no_val).show();
				$(".minus_chk"+no_val).show();
			}
	 ...
	 $(document).ready(function(){
		...
		$(".arrow_top").click(function(){
			...
			var no_val = $(this).parent().parent().next().find("input[type=file]").attr("name").replace("file","");
			$(".minus_chk"+no_val).show();
			fileChange()
		});
					...
					$.ajax({
						...
						success: function (data) {	
							if (trim(data) == "SUCCESS"){									
							    ...
							    $("#file_o"+no_val).val("");
								if (no_val == 4 || no_val == 8)
								{								
									$(".plus_chk"+no_val).hide();
									$(".minus_chk"+no_val).hide();
								}
								else
								{
									$(".img_tap"+no_val).hide();
...
//-->
			...
			<?function fnFileRender($start,$end, $colspan ,$label,$row, $no_btn=false, $view_arrow=false){
					for ($i = $start;$i<= $end; $i++){
						...
						?>
						<td>
						<div class="img-cntrl-wrap img_tap<?=$i?>" <?if ($i>=4 && !$filename && $i!=$start){?>style="display:none;"<?}?>>
							<?
							...
							?>
							<span class="img-wrap <?=substr($column_nm,0,4)?>"><img alt="" id="fileimg<?=$i?>" <?=get_noimg_photo($file_path, $filename, "/kor/images/company_".substr($column_nm,0,4).".gif")?> width="<?=$imgwsize?>" height="<?=$imghsize?>" ></span>
							<input name="file_o<?=$i?>" id="file_o<?=$i?>" type="hidden" value="<?=$fileonly?>">
							<input name="file<?=$i?>" id="file<?=$i?>" type="file" style="width:1px;height:1px;background-color:#fff;border:none;position:absolute;left:50%;top:0">
							<a href="javascript:;" class="arrow_top <?=$plus_chk?>" style="<?=$display_b?>">+</a>
							<a href="javascript:;" class="arrow_bottom <?=$minus_chk?>" style="<?=$display_b?>">-</a>
							<?
							}else{
							?>
