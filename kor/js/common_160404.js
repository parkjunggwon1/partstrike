...
*** 2016-04-03 : updateQty() 함수 수정, form 'f'를 'f_04_05'로 변경
*********************************************************************************/
...
//						alert_msg("구매자에게 송장을 발송했습니다.");
			...
			$.ajax({ 
				...
				success: function(data){
					if($.trim(data)=="ERR"){
						...
						openLayer("layer5","30_05","?odr_idx="+data); //P.O
	...
	$("body").on("click",".btn-view-sheet-0120",function(){
		if(parseInt($("#psb").val().replace(/,/gi,""))<parseInt($("#rqst_amt").val().replace(/,/gi,"")) || $("#rqst_amt").val()==""){
			alert_msg("금액을 다시 확인해 주세요.");
		}else{
			openLayer("layer5","23_23","?rqst_amt="+$("#rqst_amt").val());
		}
	});

	$("body").on("click",".btn-view-sheet-0120-r",function(){
			openLayer("layer5","01_20","?rqst_amt="+$("#rqst_amt").val());
	});
...
//		openLayer("layer3","19_05","");
	...
	$("body").on("click",".btn-dialog-0501",function(){
		var err = false; 
		err = updateQty('05_04'); //2016-04-03 '05_04' 추가
		if (err == false)
		{
			saveExtraInfo();
	...
	$("body").on("click",".btn-dialog-save",function(){
			...
			if (err == false)
			{				
				//$(".layer3-section #save_yn").val("Y");
				f.save_yn.value = "Y";
				//$(".layer3-section #typ").val("odredit");
				f.typ.value = "odredit";
				maskoff();
				//var f =  document.f; 
...
//		openLayer("layer3","19_17");
	...
	$("body").on("click",".btn-order-confirm",function(){
		...
		maskoff();
		err = updateQty('05_04');	//수량변경(ajax-UQ), 발주수정(ajax-odr_proc.php?typ=odredit)
		if (err == false)
		{
			if(det_cnt==1){
			...
			if($chked_odr.length==0){ 
				...
				openCommLayer("layer4","30_04","?odr_idx="+$("#odr_idx_05_04").val()+"&new_odr_idx="+new_odr_idx+"&whole_part_type="+$("#whole_part_type").val());
			}
		}		
	});
	...
	$("body").on("click",".periodreq",function(){
		...
		var f =  document.f;
		 ...
		 f.action = "/kor/proc/odr_proc.php";
		 f.submit();
		 //if(frompage != "add") window.location.href="/kor/"; //2016-03-31
		 //if(frompage != "add") Refresh_Right(); //2016-03-31
		 if(frompage != "add") showajaxParam('.col-right','side_order',''); //2016-03-31
	});
...
function updateQty(fromPage){
	...
	if (err == false)
	{
		...
		$.ajax({
				...
				success: function (data) {
					/**
					if($("#odr_idx_"+$("#loadPage").val()).parent().html().indexOf("확인중")>0){
						err =true;
						alert_msg("납기 확인중인 부품이 있습니다.");							
					}else{
						err=false;
