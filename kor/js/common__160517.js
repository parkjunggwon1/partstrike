...
//						document.location.href="/kor/";
	...
	$("body").on("click",".buy-mn03",function(){ 
		...
		$.ajax({
				...
				data: "typ=invconfirm2&odr_idx="+$("#odr_idx_30_09").val()+"&sell_mem_idx="+$("#sell_mem_idx_30_09").val(),  //2016-04-15 : invconfirm2(품목 Log 기록)로 변경
				encType:"multipart/form-data",
				success: function (data) {	
					if (trim(data) == "SUCCESS"){		
	...
	$("body").on("click",".btn-view-sheet-1207",function(){
		...
		if($chked_odr.length==0){ 
			...
			if (err == false)
			{
				...
				$.ajax({
						url: "/kor/proc/odr_proc.php", 
						data: "typ=poano&odr_idx="+odr_idx,
						encType:"multipart/form-data",
						success: function (data) {	
							if (trim(data) == "SUCCESS"){						
...
//		openLayer("layer3","19_17");
	...
	$("body").on("click",".btn-pop-18R04",function(){
		...
		if($chked_odr_det.length==0){
			...
			//마자막 물품인지 체크하여 운임창(18R_04)).. 아니면, 그냥 거절창(18R_05)
			if(det_cnt>1){  //남은게 있다..
				//openLayer("layer3","18R_05","?odr_idx="+$("#odr_idx_30_20").val()+"&odr_det_idx="+ch_odr_det_idx); 2016-05-16
				//수량 입력 창
				openLayer("layer4","19_04","?odr_idx="+$("#odr_idx_30_20").val()+"&odr_det_idx="+ch_odr_det_idx); //2016-05-16
			}else{	//남은게 없다.(마직막)
	...
	$("body").on("click",".btn-pop-0201",function(){
		alert3("삭제","삭제 시 해당 품목은 당신의 재고 목록에서 삭제됩니다. 만약 해당 품목을 판매하고 싶다면 재 등록해 주시기 바랍니다.","btn_end","31_05","Y"); //2016-04-07
	...
	$("body").on("click",".btn-pop-1904",function(){
		...
		if($chked_odr_det.length==0){
			...
			openLayer("layer4","19_04", "?odr_det_idx="+ch_odr_det_idx+"&fault_method="+$(this).attr("fault_method"));
		}
	});
