...
switch($actty) {
   ...
   break;
   case "agent":
	   fnagent($actty,$strsearch,$strsearch1); 
   break;
   ...
   break;
   case "recordlist":
   	   fnRecordList($odr_type, $part_no,$yr,$mon,$this_mem_idx);
   break;
   case "sideodrlist":
   ...
   case "GDA": //get delivery addr
		echo GET_CHG_ODR_DELIVERY_ADDR($actidx);
   break;
   case "mem":
...
//		echo $sql;
   ...
   case "CF":
	   ...
	   $sql = "insert into odr_history set 
		...
		break;
	case "CFO": //CF only   정상 종료가 아닌 경우에는 (환불) 모든 것을 다 처리 하고 종료 할때에는 confirm_yn 만 y로 변경해준다.
		update_val("odr_history","confirm_yn","Y", "odr_history_idx", $actidx);
	break;
	case "DP": 
		// 첫 지연 처리	   
   ...
   case "EP" : //rnd 후 종료 프로세스
	...
	if($tTy =="S"){
		...
		if ($winner == "S") {
			...
			$sql = "insert into mybank (mem_idx, rel_idx, charge_type, charge_amt, charge_method, odr_idx, odr_det_idx,  memo, reg_date, reg_ip)
				select $mem_idx, $rel_idx , '16', -1*charge_amt, 'MyBank', odr_idx, odr_det_idx, memo, now(), reg_ip from mybank where odr_idx=".$fty_his[odr_idx]." and odr_det_idx=".$fty_his[odr_det_idx]." and charge_type=13 and mem_idx=".$fty_his[sell_mem_idx];
			$result=mysql_query($sql,$conn) or die ("SQL ERROR : ".mysql_error());
		}else{
			//seller의 운임료를 차감.
		...
		$my_idx = get_any("mybank", "mybank_idx ", "odr_idx=".$fty_his[odr_idx]." and odr_det_idx=".$fty_his[odr_det_idx]." and charge_type=13 and charge_amt<0 ");
	}else{
		...
		if ($winner == "B") {			
			...
			$result=mysql_query($sql,$conn) or die ("SQL ERROR : ".mysql_error());		
			$new_idx=mysql_insert_id(); 
			//-값이 한번에 안들어가길래 insert후 -를 업데이트함.
			$sql = "update mybank set charge_amt = charge_amt * -1 where mybank_idx =$new_idx";
			$result=mysql_query($sql,$conn) or die ("SQL ERROR : ".mysql_error());		

			$sql = "insert into mybank (mem_idx, rel_idx, charge_type, charge_amt, charge_method, odr_idx, odr_det_idx,  memo, reg_date, reg_ip)
				select $mem_idx, $rel_idx , '16', -1*charge_amt, 'MyBank', odr_idx, odr_det_idx, memo, now(), reg_ip from mybank where odr_idx=".$fty_his[odr_idx]." and odr_det_idx=".$fty_his[odr_det_idx]." and charge_type=11 and mem_idx=".$fty_his[buy_mem_idx];
				//echo $sql."<BR>";
				//exit;
			$result=mysql_query($sql,$conn) or die ("SQL ERROR : ".mysql_error());
		}else{
			//운임료 차감 : buyer는 운임료 차감을 안한다. 왜냐하면 물품을 보낼 때 buyer가 미리 냈으므로. 
		}
		$my_idx = get_any("mybank", "mybank_idx ", "odr_idx=".$fty_his[odr_idx]." and odr_det_idx=".$fty_his[odr_det_idx]." and charge_type=11 and charge_amt<0 ");
...
//	echo $my_idx."~~~~~~~~~";
	if (QRY_CNT("mybank","and odr_idx=".$fty_his[odr_idx]." and odr_det_idx=".$fty_his[odr_det_idx]." and charge_type in(11,13) and charge_amt<0 and chk='Y'")=="2"){
		$sql = "update fty_history set confirm_yn='Y' where fty_history_idx=$actidx";
		$result=mysql_query($sql,$conn) or die ("SQL ERROR : ".mysql_error());		
	}
   break;

   case "MRO" :   //Move To Real Order (임시 order에서 real order로 발주 들어가는 과정 -----------------------------------(실 발주)--------------------------------------------------/
	...
	if($safe_stock>0){ //-- 재고 부족 -------------------------------------------------
		...
		$sql = "UPDATE part AS a 
				...
				WHERE b.odr_det_idx IN($actkind)
				";
		$result=mysql_query($sql,$conn) or die ("SQL ERROR : ".mysql_error());
		//-- 발주서 처리--------------------------------------------------------------------
   ...
   case "NORD" :   //2015-03-23 : 납기 품목 있는 발주 창에서 납기 삭제 후, 나머지는 새 발주번호(ord)-------------------------------------
		//선택한 부품들은 새로운 odr_idx를 따서 옮겨야 함.
   ...
   case "POCS" :   //2016-04-13 : 판매자 송장화면(30_08->po_cancel)에서 선택 품목 취소 --------------------------------------------------------------------------
		//0. 기존 odr 정보 열람
		$odr = get_odr($odr_idx);
   ...
   break;
	...
	echo ":::".GET_SEl_BOX("opt1", TRUE, $area=="on"?"City/Province":"Nation", $sel_nation,"", $area=="on"?$areaonsrch:$opt_shand,$area).":::".GET_SEl_BOX("opt2", TRUE, "Manufacturer", $sel_manufacturer,"", $searchand,$area);
	break;
	...
	case "PMI" : // put money in (입금 처리)
		...
		if (($mybank[deposit_yn] =="Y" && -$mybank[charge_amt] !=1000) || $mybank[deposit_yn] !="Y"){
			...
			$sql = "insert into odr_history set 
						...
						,status = 5
						,status_name = '결제완료'
						,etc1 = '$".number_format(-$mybank[charge_amt],2)." 입금'
		...
		//exit;
	break;
	case "PMIF" : // put money in (입금 처리) - faulty쪽.  status = 27
		$sql = "update mybank set put_money_yn = 'Y' where mybank_idx = $mybank_idx";
		$result=mysql_query($sql,$conn) or die ("SQL ERROR : ".mysql_error());
		$mybank = get_mybank($mybank_idx);
		//echo "~~~~".$mybank[deposit_yn];
...
function fnRecordList($odr_type, $part_no,$yr,$mon,$this_mem_idx){
	echo GF_GET_RECORD_LIST($odr_type, $part_no,$yr,$mon,$this_mem_idx);
}
...
function fnRemitList($yr,$mon,$remit_ty){
	echo GF_GET_REMIT_LIST($yr,$mon,$remit_ty);
}
...
function fnagent($actty,$strsearch,$strsearch1){
	 include ($_SERVER["DOCUMENT_ROOT"]."/kor/include/$actty.php");
}
