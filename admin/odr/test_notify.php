...
<!--
	function close_it(lay_yn,fty_history_idx){
		if (lay_yn=="y"){
			$("#fty_his_"+fty_history_idx+" option:eq(0)", parent.document).attr("selected",true);
			parent.document.getElementById("popdiv").style.display="none";
		}
		if (lay_yn=="n"){
			//$(opener.document).find("#fty_his_"+fty_history_idx+" option:eq(0)").attr("selected",true).submit();
			$("#fty_his_"+fty_history_idx+" option:eq(0)", opener.document).attr("selected",true);
			self.close();
		}
...
<tr><td>* Test Result</td>
	<td>
		<input type="radio" name="test_result" value="OK">OK <input type="radio" name="test_result" value="Fail">Fail
	</td>
 </tr>
 <tr><td>* Result Comment</td>
	...
	<tr>
		...
		<td width="50%" align="right">
			<span  class="button1" onClick="check();" style="cursor:pointer;">결과공지</span>
			<span  class="button1" onClick="close_it('y','<?=$fty_history_idx;?>')" style="cursor:pointer;">닫기</span>&nbsp;
		</td>
	</tr>
	</table>
 </body>
