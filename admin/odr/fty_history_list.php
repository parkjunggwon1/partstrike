...
$cnt = QRY_CNT("odr a left outer join fty_history h  on a.odr_idx = h.odr_idx left outer join member s on a.sell_mem_idx = s.mem_idx
		...
		if (obj.value=="Y")
		{
			...
			if(confirm("정말 입금 처리 하시겠습니까?")){
				$.ajax({
					...
					data: { actty : "PMIF", //Put money in (입금 : deposit(보증금)이랑 헷갈릴까봐.)
