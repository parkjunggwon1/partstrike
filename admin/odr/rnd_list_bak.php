...
$searchand=" and a.odr_no <>'' and fty_history_idx <> ''";
if ($strsearch){
	$searchand .= " and $search like '%$strsearch%' GROUP BY h.odr_det_idx";
}

$cnt = QRY_CNT("odr a left outer join fty_history h  on a.odr_idx = h.odr_idx and h.status IN(25,28,14,30) and h.reason_ty='6' left outer join member s on a.sell_mem_idx = s.mem_idx
...
$tbl = "odr a 
left outer join fty_history h  on a.odr_idx = h.odr_idx and h.status IN(25,28,14,30) and h.reason_ty='6'
...
$result =QRY_C_LIST($c,$tbl,$recordcnt,$page,$searchand," h.fty_history_idx DESC");
?>
...
<!--	
	...
	function popup_view(his_idx, ship_idx){
		...
		if(lay_yn=="y"){
			document.getElementById("popifr").src="test_notify.php?fty_history_idx="+his_idx+"&ship_idx="+ship_idx;
		...
		if(lay_yn=="n"){
			window.open("test_notify.php?idx="+id,"_blank","left="+p_left+",top="+p_top+",scrollbars="+scr_yn+",resizable=no,copyhistory=no,width="+p_wid2+",height="+p_hei2+"");
		}
	}
//-->
</script>
				...
				<table width="100%" border="0" cellspacing="0" cellpadding="0">
					...
					<tr>
						...
						while($row = mysql_fetch_array($result)){
							$fty_idx = $row["fty_history_idx"];
							$c2 = "h.*,s.mem_idx s_mem_idx, s.mem_nm s_mem_nm ,s.mem_nm_en s_mem_nm_en, s.rel_idx s_rel_idx, s.nation s_nation,b.mem_idx b_mem_idx,b.mem_nm b_mem_nm,b.mem_nm_en b_mem_nm_en, b.rel_idx b_rel_idx, b.nation b_nation, a.odr_idx, a.odr_no";
							$tbl2 = "odr a 
									left outer join fty_history h  on a.odr_idx = h.odr_idx h.fty_history_idx=$fty_idx
									left outer join member s on a.sell_mem_idx = s.mem_idx
									left outer join member b on a.mem_idx = b.mem_idx
									";
							$result =QRY_FTY_VIEW($c2, $tbl, " h.fty_history_idx DESC");
							/**
						...
						<table width="99%" border="0" cellspacing="0" cellpadding="0">
							...
							<tr>
								...
								<td width="150" align="center">
									<select id="fty_his_<?=$fty_history_idx;?>" onchange="changeTestVal(this , <?=$fty_history_idx;?>)">
										<option value=''></option>
										<option value='28'>수령</option>
										<option value='OK'>결과:OK</option>
										<option value='FAIL'>결과:Fail</option>
									</select>
								</td>
								<!--td width="150" align="center">
								<?if ($status == "13"){							
...
<!--
	...
	function changeTestVal(obj, fty_history_idx){
		if (obj.value=="28")
		{
