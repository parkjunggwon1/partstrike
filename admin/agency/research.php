...
$cnt = QRY_CNT("research",$searchand);
$totalpage = QRY_TOTALPAGE($cnt,$recordcnt);
$result =QRY_LIST("research",$recordcnt,$page,$searchand," idx desc");
...
<!--
	...
	function check(obj,typ,txt){
		...
		if (nullchk_admin(f.delivery,"운송회사를 선택하세요.")== false) return ;
		if (nullchk_admin(f.account_no,"Account No를 입력하세요.")== false) return ;
		
		f.typ.value=typ;
		f.target = "_self";
		f.action = "/admin/agency/research_proc.php";
		f.submit();
	}
//-->
</script>
				...
				<table width="100%" border="0" cellspacing="0" cellpadding="0">
					...
					<tr>
						...
						<table width="99%" border="0" cellspacing="0" cellpadding="0" background="/admin/img/boardbar_bg.gif">
							<tr>
								<td height="27" align="center" class="btitle01" >운송회사</td>
								<td width="250" align="center" class="btitle01">Account No</td>
						...
						<table width="99%" border="0" cellspacing="0" cellpadding="0">
							<tr>
								...
								<option value="">운송회사</option>
								<?
								$result1 =QRY_LIST("code_group_detail","all","1"," and grp_code='DLVR' and code_depth='1' "," dtl_code ASC");
								while($row1 = mysql_fetch_array($result1)){
									$dtl_code= replace_out($row1["dtl_code"]);
									$code_desc= replace_out($row1["code_desc"]);
								?>
								<option value="<?=$dtl_code?>" ><?=$code_desc?></option>
								<?
								}
								?>
								</select>
								</td>	
								<td width="250" align="center"><input name="account_no" type="text" maxlength="50" value="<?=$account_no?>" class="inputtext"></td>
								<td width="250" align="center"><input name="email" type="text" maxlength="50" value="<?=$email?>" class="inputtext"></td>
								<td width="250" align="center"><span class="btn1"><a href="javascript: check('f','write','저장');">추가</a></span></td>
							</tr>
							<tr>
						...
						<table width="99%" border="0" cellspacing="0" cellpadding="0" background="/admin/img/boardbar_bg.gif">
							<tr>
								...
								<td width="250" align="center" class="btitle01">Account No</td>
								<td width="250" align="center" class="btitle01">Email</td>				
								<td width="250" align="center" class="btitle01">상태변경</td>
							</tr>
						</table>					
						<!--일반게시글 시작-->
						<?
						...
						<table width="99%" border="0" cellspacing="0" cellpadding="0">
							<tr>
								...
								while($row1 = mysql_fetch_array($result1)){
									...
									$code_desc= replace_out($row1["code_desc"]);
								?>
								<option value="<?=$dtl_code?>"  <?If ($dtl_code==$delivery) {?> selected <? } ?>><?=$code_desc?></option>
								<?
								}
								?>
								</select>
						...
						<br />
						<form name="searchfrm" method="post" action="research.php?mode=<?=$mode?>">
						<input type="hidden" name="mode" value="<?=$mode?>">
