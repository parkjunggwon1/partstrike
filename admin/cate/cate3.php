...
<!--
	...
	function go(ct1,ct2){
		parent.document.getElementById("cate4").src="cate4.php?cate1_code="+ct1+"&cate2_code="+ct2;
	}
...
If ($title!=""){
	...
	$sql = "insert into  code_group_detail (grp_idx, grp_code, dtl_code,dtl_par_code, code_seq, code_depth,  code_desc$lang, use_yn)
			values($grp_idx , '$cate1_code' , '$code','$cate2_code','$code' , 2, '$title', 'Y')";
	//	echo $sql;
...
<form name="f" method="post" action="cate3.php" target="cate3">
...
<table width="100%" border=0>
	...
	<tr>
		<td class="board02" colspan="3" style="height:60px;">
			...
			<a href="javascript:go_sort(3 , '<?=$cate1_code?>' ,'<?=$cate2_code?>')" target="cate3"><span class="button1">순서변경</span></a>
			<?if ($lang==""){?>
...
<?}else{?>
	<table width="100%" border=0>
		<tr>
			<td class="board02">대분류(1단계) 카테고리를 먼저 선택해주세요</td>			
		</tr>
	</table>
<?}?>
